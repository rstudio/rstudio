...
grep -rohE $EXCLUDE "\b($R_MACROS)\b" "$RSTUDIO_CPP" 2>/dev/null | sort -u > "$TMPDIR/macros.txt"
...
if [ -s "$TMPDIR/result_nonapi.txt" ]; then
  ...
  printf "%-35s %s\n" "------" "----"
  while read -r sym; do
    count=$(grep -rw $EXCLUDE "$sym" "$RSTUDIO_CPP" 2>/dev/null | grep -v '^\s*//' | grep -v '^\s*\*' | wc -l | tr -d ' ')
    if [ "$count" -gt 0 ]; then
      printf "%-35s %s\n" "$sym" "$count"
    fi
