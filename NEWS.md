...
- ([#17912](https://github.com/rstudio/rstudio/issues/17912)): Add a `project-trust-required` session option (off by default) that treats projects as untrusted by default, prompting users to trust each project when opened even if it contains no auto-executing files. In untrusted projects, the visual editor is disabled; only the source editor is available for markdown documents.
