...
|---|---|---|---|
| `RS_NO_SPLASH` | Non-empty | Desktop | Disables the splash screen shown while RStudio is starting up. There is also a setting for this in **Tools** > **Global Options** > **General**. |
...
| `RSTUDIO_DISABLE_POSIT_ASSISTANT_INSTALLATION` | Non-empty | Desktop | Prevents you from installing, updating, or uninstalling [Posit Assistant](../guide/tools/posit-ai.qmd) yourself. RStudio uses only the installation your administrator provides, ignores any installation in your own user directory, and does not check for updates. Posit Workbench administrators use the `allow-posit-assistant-installation` option in `rsession.conf` instead. |
...
| `RSTUDIO_DISABLE_WHATS_NEW` | Non-empty | Desktop | Suppresses the "What's New" dialog that is otherwise shown after RStudio is updated to a new version. There is also a setting for this in **Tools** > **Global Options** > **General**. |
