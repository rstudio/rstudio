...
- Editor commands
- User defined R functions or [RStudio Addins](https://rstudio.github.io/rstudioaddins/) or the [`shrtcts`](https://pkg.garrickadenbuie.com/shrtcts/) package

Access the keyboard shortcuts from the RStudio menu: **Tools** > **Modify Keyboard Shortcuts**:

![](images/keyboard-shortcuts.png){fig-alt="A screenshot of the Tools menu bar highlighting the Modify Keyboards Shortcuts"}

Seleting **Modify Keyboard Shortcts** will display available editor commands (commands that affect the current document's contents, or the current selection), alongside RStudio commands (commands whose actions are scoped beyond just the current editor).
...
After a keboard shortcut has been modified, press `Apply` and the shortcuts will be applied to the current session and saved for future sessions.
