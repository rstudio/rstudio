...
Note that RStudio will not merge snippet files.
...
For Quarto and RMarkdown documents, templates should not have a YAML header. The **File** > **New File** workflow provides a popup wizard where users select specific options that populate the YAML header.
...
For more information on creating custom shortcuts, see the [RStudio Custom Shortcuts section](../productivity/custom-shortcuts.qmd).
