...
If an `ai.prev` directory is present beside `pai`, left by an interrupted install or update, delete it as well. If `RSTUDIO_DATA_HOME` is set, the directory is `$RSTUDIO_DATA_HOME/pai`. If `XDG_DATA_HOME` is set instead on macOS or Linux, it is `$XDG_DATA_HOME/rstudio/pai`.

This directory holds the downloaded Posit Assistant program files and RStudio's record of its update checks. Posit Assistant keeps your chat history and settings under `~/.posit`, outside this directory, and they remain available if you install Posit Assistant again.
