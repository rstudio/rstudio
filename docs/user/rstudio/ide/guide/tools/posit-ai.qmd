...
This directory holds the downloaded Posit Assistant program files -- one directory per installed version under `versions`, plus `selected.json` naming the version in use, and a `bin` directory if an earlier RStudio release installed there -- and RStudio's record of its update checks. Each install adds a version directory and removes nothing, so the directory grows with each update until it is deleted. Posit Assistant keeps your chat history and settings under `~/.posit`, outside this directory, and they remain available if you install Posit Assistant again.
