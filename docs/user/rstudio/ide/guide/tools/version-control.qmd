...
3.  Press **Enable version control interface for RStudio projects**

4.  If necessary, enter the path for the Git or SVN executable where provided. A RSA key can be created or added for SSH if necessary.
...
![](images/git-tab.png){fig-alt="A screenshot of the Git tab in RStudio, showing a few files that can be added to staging." width="450"}
...
![](images/git-commit-pane.png){fig-alt="A screenshot of the Review Changes popup, indicating the files for staging and the ability to commit, push and pull changes."}
