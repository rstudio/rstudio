...
::: callout-note
If you've installed Git for version control, but the Git pane doesn't appear you may need to perform the steps outlined in the section [Git is installed, but the Git pane doesn't appear].
:::
...
The Git pane may not appear if you're working in a folder that your user does not have ownership of, such as a shared project. This is due to a security feature in git `2.35.2` that disables git repos that are created in directories that are not owned by the user. You can verify that this is blocking the Git pane from appearing by submitting a git command in the terminal:
...
You can mark the offending directory as safe by following the suggestion action above and then closing and re-opening the project in RStudio.
