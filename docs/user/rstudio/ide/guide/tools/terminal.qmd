...
-   If the tab isn't visible, show it via **Tools** \> **Terminal** \> **Move Focus to Terminal** (or use the kebyoard shortcut <kbd>Shift+Alt+T</kbd>).

-   The full list of terminal-related keyboard shortcuts is in the Reference: Keyboard Shortcuts [Terminal section](../../reference/shortcuts.html#terminal). Most terminal-related commands can be found on the **Terminal dropdown** menu in the terminal pane, and on the **Tools-\>Terminal** menu.
...
RStudio supports multiple terminal sessions. To start another terminal session, use the **New Terminal** command on the Terminal dropdown menu, or <kbd>Alt+Shift+R</kbd>.
...
1. Using the arrows next to the drop-down menu  
2. Clicking on the terminal's name in the drop-down  
3. Using the **Previous Terminal**/**Next Terminal** drop-down menu commands. The dropdown button shows the name of the currently selected terminal session ("Terminal 1" in the following screenshot):  
