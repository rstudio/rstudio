...
If the function object doesn't exist yet (for instance, because `source()` hasn't been called on the file), or the function object doesn't match the contents of the editor (for instance, because the file has changed since the last `source()`), RStudio defers the breakpoint.
...
The R function [`browser()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/browser.html) halts execution and invokes an environment browser when it is called. You can add a `browser()` call anywhere in the code to stop at that point for debugging. Here, for instance, it's used to halt when a function is about to return `TRUE`:

![](images/browser-debug.png){fig-alt="A screenshot of R code with a browser() call, along with the traceback."}

Unlike an editor breakpoint, the `browser()` statement is actually part of the code, so you need to apply it like any other code change in order to become active (by sourcing the containing document, rebuilding the containing package, reloading the Shiny application, etc.).
...
![](images/debug-package-tools.png){fig-alt="A screenshot of the Project Options dialog, with Build Tools selected and the Package Build Options showing the --with-keep.source option enabled."}
