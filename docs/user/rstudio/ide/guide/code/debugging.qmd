...
Debugging in R is a broad topic. In this article, we focus specifically on the R debugging tools built into the RStudio; for more general advice on debugging in R (such as philosophy and problem-solving strategies), we recommend this resource from Hadley Wickham: [Debugging from Advanced R](https://adv-r.hadley.nz/debugging.html)

Copied from <https://support.rstudio.com/hc/en-us/articles/205612627-Debugging-with-the-RStudio-IDE>
...
Use the R function [`debugonce()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/debug.html) to set the debug flag on a function. For instance, if you want to debug `devtools::install()`:
...
Finally, because R Markdown chunks don't contain [source references](http://journal.r-project.org/archive/2010-2/RJournal_2010-2_Murdoch.pdf), most of the debugger's visual features are disabled; you won't see the active line highlighting in the editor and most debugging will need to be done in the console.
