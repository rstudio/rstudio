...
RStudio supports the automatic completion of code using the **Tab** key. For example, if an object named `pollResults` exists in the workspace, typing `poll` in the R console and then pressing **Tab** will automatically complete the full name of the object.
...
To review a list of recently executed commands use the <kbd>Ctrl+Up</kbd> to review the list (on Mac <kbd>Command-Up<kbd>) and then select a command from this list by pressing <kbd>Enter</kbd>:
...
The below screenshot illustrates a few additional capabilities provided by the Console title bar:
