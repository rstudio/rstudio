...
RStudio supports the direct execution of code from within the source editor (RStudio inserts the executed commands into the console where the output also appears).
...
2.  After executing a selection of code, use the **Re-Run Previous Region** command (or its associated toolbar button) to run the same selection again. Note that RStudio reflects any changes to the selection including additions, removals, and modifications of lines in this subsequent run of the selection.

3.   To run the entire document press the <kbd>Ctrl+Shift+Enter</kbd> keys (or use the **Source** toolbar button).

The difference between running lines from a selection and invoking **Source** is that when running a selection all lines are inserted directly into the console whereas for **Source** RStudio saves the file to a temporary location and then sources it into the console (thereby creating less clutter in the console).

![](images/re-run-source.png){fig-alt="Highlighting the Run, Run previous, and Source menu options in the Source pane."}

### Source on Save

When editing re-usable functions (as opposed to freestanding lines of R) you may wish to set the **Source on Save** option for the document (available on the toolbar next to the Save icon). Enabling this option makes RStudio automatically source the file into the global environment every time you save it.
...
1.  Select the lines and press the <kbd>Ctrl+Enter</kbd> key (or use the **Run** toolbar button); or

2.  To run the entire document press the <kbd>Ctrl+Alt+R</kbd> keys (or use the **Run All** toolbar button).

3.  To run the entire document and convert it to the final output, use the **Knit** or **Render** button.

4.  To run an entire cell, press the "Run Current Chunk" button on the code chunk, or in the **Run** toolbar via "Run Current Chunk", or with the <kbd>Ctrl+Shift+Enter</kbd> shortcut
