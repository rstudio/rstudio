...
Upon startup, R and RStudio look for a few different files used to control the behavior of the R session, for example by setting options or environment variables. In the context of Posit Team, these settings are often used to set RStudio Workbench (previously RStudio Server Pro) to search for packages in an Posit Package Manager repository.
