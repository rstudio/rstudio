...
RStudio v2022.07.1 and beyond includes support for editing and preview of [Quarto documents](https://quarto.org/).
...
To upgrade Quarto out of sync with the bundled RStudio version, follows the directions at <https://quarto.org/docs/download/>
...
The preview will appear alongside the editor in the **Viewer** pane:
...
Quarto documents rendered in RStudio with the **Render** button or the **Render on Save** option are rendered within a [Background Job](../tools/jobs.html) which maintains a background preview web server. This provides a fresh R session for the rendered document along with a consistent URL to access the preview content.

To access and control the Quarto render, navigate to the **Background Jobs** tab of the **Console** pane:

- The duration of the current preview web server is available in the top right corner
...
- To interrupt the render, press the red **Stop** button in the top right corner of **Background Jobs tab
