...
The RStudio user interface has 4 primary panes and an optional sidebar pane:
...
-  **Sidebar pane**, initially empty and hidden but can be configured to show any of the tabs from the **Environment pane** or the **Output pane**, and can be displayed to the right (default) or left of the other panes
...
![](images/rstudio-panes-labeled.png){fig-alt="A screenshot of the four RStudio panes, labeled Source, Environments, Console, and Output, and the Sidebar toggle button"}
Clicking the Sidebar toggle button displays the Sidebar pane.

![](images/rstudio-panes-sidebar.png){fig-alt="A screenshot of the four RStudio panes and the default empty Sidebar"}
...
Additional source "columns" can be added to the left of the four primary panes. Arbitrary editor placement (for example, placing source columns to the right of the primary panes) is not supported. To add additional source columns, from the RStudio menu: **Global Options** \> **Pane Layout** \> **Add Source Column**, which provides the ability to work on two (or more) source files at a time within the Source Pane.
...
Additional source "columns" can be added to the left of the four primary panes. RStudio uses a fixed four-pane layout. The Source pane currently supports splitting only leftward within its allocated region. Arbitrary editor placement (for example, placing multiple Source panes to the right of the Console) is not supported. To add additional source columns, from the RStudio menu: **Global Options** \> **pane Layout** \> **Add Source Column**, which provides the ability to work on two (or more) source files at a time within the Source pane.
...
By default, the Sidebar pane is hidden, and can be displayed using the Sidebar toggle button or View / Show Sidebar. The Sidebar can be configured to display on the left, if desired, and to show any of the tabs from the Environment or Output panes.
