...
To install it, see [Download RStudio](download.qmd).
...
RStudio Desktop, RStudio Server, and the commercial editions all have installers on
[Download RStudio](download.qmd), along with a note about the
[daily builds](https://dailies.rstudio.com/).

<!-- Fragment targets for links written against the download sections that used to
live on this page. Anchors are never sent to the server, so no redirect can catch
them; these keep such links landing on the section that points at the new page. -->
[]{#open-source}[]{#commercial}[]{#direct-downloads-open-source}[]{#rstudio-ide-oss-downloads}[]{#rstudio-server-oss-downloads}[]{#direct-downloads-commercial}[]{#daily-builds}
