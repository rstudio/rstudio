...
This page walks through an example of using the `ggplot2` and RStudio for basic data visualization. If you are new to R and want to learn more about using R for Data Science, also see the [R for Data Science](https://r4ds.had.co.nz) textbook which is available in [print](https://www.oreilly.com/library/view/r-for-data/9781491910382/) or as a [freely available website](https://r4ds.hadley.nz/)
...
RStudio is a set of integrated tools designed to help you be more productive with R and Python.
...
1.  The **source pane** is where you can edit and save R or Python scripts or even author computational documents like Quarto.
...
![](rstudio-panes-labeled.jpeg){fig-alt="A screenshot of the RStudio UI. There are 4 primary panes, the source, console, environment, and output panes."}
...
The following section is adapted from the ["Data Visualization" chapter](https://r4ds.had.co.nz/data-visualisation.html) of R for Data Science.
...
-   Using the **File Menu** in RStudio, create a new .R file and label it `ggplot2.R`

The `ggplot2.R` file you have just created will be in the **Source Pane**, which by default is in the top left.

Once you have successfully installed `ggplot2`, you can then write the following code into the first line of your `ggplot2.R` script.
...
You can then see the first few rows of the `mpg` dataframe like so:
...
![](basic-car-plot.png){fig-alt="A ggplot of fuel displacement on the x-axis and highway miles per gallon on the y-axis. There is a negative, roughly linear relationship between mpg and displacement, as fuel displacement increases, the highway mpg decreases. The points are colored by class of vehicle, where pickups and SUVs have larger engines and worse fuel-efficiency than subcompact, compact, or midsize vehicles."}

Again, you will want to execute that by either the Run command or shortcut (<kbd>`Ctrl + Enter`</kbd> or <kbd>`Cmd + Enter`</kbd> on Mac). Note that we are using the `<-` operator to assign this plot to the `mpg_plot` object. You can see the object name in the top right **Environment Pane**.
...
```r
ggsave("my-first-plot.png", plot = mpg_plot)
```
...
You should also save your `ggplot2.R` file at this point. From the **Files Tab** you can find your saved `ggplot2` graph, and if you have saved your `ggplot2.R` script - it will also be in this same folder.

Remember - with your R scripts and data files you can recreate the temporary session environment! To prove our point, after you have saved your `ggplot2.R` file, restart your session with the RStudio menu: **Session > Restart R**. Even in this fresh environment, you can recreate your plot by re-executing the source code with the same data!
