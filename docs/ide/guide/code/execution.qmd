...
Similar to source code files, code can be executed via RStudio "Run" commands or keyboard shortcuts <kbd>Ctrl + Enter</kbd>. However, each chunk also has a "Run" button to execute that specific chunk.

![](images/document-chunk.png){fig-alt="A screenshot of a R code chunk in Quarto. It is primarily 3x backticks and a language engine."}
