...
document.addEventListener("DOMContentLoaded", function () {
  const issuePattern = /(?:^|\s|[\(\[,;])((?:[a-zA-Z0-9\-]+)?#\d+)(?=[\]\),;:\s]|$)/g;

  document.querySelectorAll("p, li").forEach(el => {
    el.innerHTML = el.innerHTML.replace(issuePattern, (match, group) => {
      return match.replace(group, `<span class="issue-ref">${group}</span>`);
    });
  });
});
</script>
