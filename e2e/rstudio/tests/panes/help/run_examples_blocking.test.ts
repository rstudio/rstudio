...
test.describe('Help "Run examples" for blocking examples', () => {
  ...
  test.beforeAll(async ({ rstudioPage: page }) => {
    ...
    // The HTML help "Run examples" link is generated by tools::Rd2HTML itself
    // (enhancedHTML branch), not by RStudio -- it doesn't exist before R 4.2.0.
    // Skipping here (rather than in the test) skips the whole describe block
    // and avoids installing the fixture package for nothing.
    const enhancedHtmlSupported = await consoleActions.evalRLogical(
      'getRversion() >= "4.2.0"',
    );
    test.skip(
      enhancedHtmlSupported !== true,
      'HTML help "Run examples" link requires R >= 4.2.0',
    );
