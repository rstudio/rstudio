...
test.describe('sandbox layout', { tag: ['@desktop_only'] }, () => {
  ...
  test('a seeded Posit Assistant is laid out as a selected version slot', async ({ rstudioSession }) => {
    ...
    // The session under test has a storage directory of its own, provisioned
    // from the seed (desktop.fixture's seedPaiIntoDataHome) rather than linked
    // to it, so an install it runs cannot reach the other specs' seed.
    const ownStorage = path.join(rstudioSession.configRoot!, 'data-home', 'pai');
    expect(fs.lstatSync(ownStorage).isSymbolicLink()).toBe(false);
    expect(fs.existsSync(path.join(ownStorage, 'selected.json'))).toBe(true);
  });
});
