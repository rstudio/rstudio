...
| `PW_SANDBOX_POSITAI_AUTH` | Both | No | How the `auth.setup` project provisions Posit AI credentials: `flow` (default) runs the OAuth sign-in using `POSIT_EMAIL`/`POSIT_PASSWORD`; `seed` copies the host's whole `~/.posit/assistant` directory (tokens plus skills/settings); `off` provisions nothing. Unset behaves as `flow`. `flow` never falls back to `seed` -- if `POSIT_EMAIL`/`POSIT_PASSWORD` are unset it logs and the Posit AI `@ai` tests skip. |
| `POSIT_EMAIL` | Both | Conditional | Posit account email for the `flow` sign-in. Required for `flow` to provision Posit AI; if unset, Posit AI `@ai` tests skip. Ignored by `seed`/`off`. |
