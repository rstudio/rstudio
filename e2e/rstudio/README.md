...
The Posit Assistant tests normally download the official assistant package into the sandbox `data-home`. To run them against a locally built assistant instead, set `PW_SEED_PAI` to an install directory and `globalSetup` seeds it into `data-home/pai` as an installed version slot (see Environment Variables).
...
| `PW_SEED_PAI` | Both | No | Path to a locally built Posit Assistant install directory (e.g. `~/.local/share/rstudio/pai`, as produced by the assistant repo's `npm run deploy:rstudio`). `globalSetup` lays it out in the sandbox `data-home/pai` the way an install does -- `versions/<version>` with its slot manifest, named by `selected.json` -- so tests exercise that local build instead of downloading the official package (rstudio/rstudio#18658; see `fixtures/pai-seed.ts`). The seed's own `bin/` is not copied: a versioned-aware RStudio never reads it. Fails loud if the path doesn't contain a well-formed `bin/package.json` and `bin/protocol.json`. The seeded build's version and protocol must satisfy the IDE under test, or it will be treated as needing an update. |
