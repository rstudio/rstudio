...
An external server's rsessions read the logged-in account's real home directory, which the harness cannot set up directly. Instead, a final auth-setup step provisions the sandbox AI credential stores **through the session**: it logs in, pushes each provisioned store into the remote home via the R console (`utils/remote-provision.ts`), and ends the session so tests attach to a fresh rsession that reads the new stores. A store already on the account is left alone when it holds a real sign-in; unauthenticated stubs, which sessions create on their own, are replaced. After the run, the `auth-teardown` project scrubs exactly the paths provisioning created -- and only those. Two caveats: (1) if the run crashes before the teardown, or the server is unreachable then, the pushed tokens remain in the remote home -- remove them by hand, and a preserved sandbox's `remote-provision-manifest.json` lists which paths; (2) the provisioning commands pass token bytes through the R console, so they land in the account's RStudio console history -- history files provisioning created are scrubbed too, but a pre-existing history database is never deleted. Use a dedicated test account for external-server AI runs (#18348).
