...
`HOME` / `USERPROFILE` point at a sandboxed `user-home/`. By default nothing is seeded there, so user dotfiles (`~/.Rprofile`, `~/.Renviron`, `~/.R/`, `~/.gitconfig`, `~/.ssh/`, etc.) are absent for tests. AI provider credentials are the exception, and the two providers are provisioned differently. **Posit AI** is handled by the `auth.setup` project (`tests/auth.setup.ts`), controlled by `PW_SANDBOX_POSITAI_AUTH`: by default (`flow`) it runs the OAuth sign-in using `POSIT_EMAIL`/`POSIT_PASSWORD` and writes a token store into the sandbox; `flow-seed` additionally copies the host's `~/.posit/assistant` content (excluding the token store) before signing in; `seed` instead copies the host's whole `~/.posit/assistant` directory; `off` provisions nothing. Its tests gate on the on-disk token store (not an env flag) via `requireAiCredentials()`, so an unsigned-in sandbox skips cleanly. **GitHub Copilot** is still copied from the host by `globalSetup` when the config directory exists (`~/.config/github-copilot`, or `%LOCALAPPDATA%\github-copilot` on Windows); its tests gate on the `PW_AI_SEEDED_COPILOT` flag set after a successful copy. Either way, a missing credential surfaces as "skipped: no credentials" rather than a 5-minute timeout. `PW_SANDBOX_NO_SEED_CREDENTIALS=1` is a global host-copy kill-switch: it blocks the Copilot copy, the Posit AI `seed` copy, and `flow-seed`'s content copy (the sign-in itself is unaffected, since it copies nothing from the host). Note that with `POSIT_EMAIL`/`POSIT_PASSWORD` configured, every run performs a live sign-in against `login.posit.cloud` -- the `setup` project runs unconditionally, even when no Posit AI test is selected. For fast local iteration, use `PW_SANDBOX_POSITAI_AUTH=seed` (if already signed in on the host) or `off`.
...
| `PW_SANDBOX_POSITAI_AUTH` | Both | No | How the `auth.setup` project provisions Posit AI credentials: `flow` (default) runs the OAuth sign-in using `POSIT_EMAIL`/`POSIT_PASSWORD`; `flow-seed` runs the same sign-in but first copies the host's `~/.posit/assistant` content into the sandbox, excluding the token store (host tokens are never copied -- the sign-in writes fresh ones); `seed` copies the host's whole `~/.posit/assistant` directory (tokens plus skills, workspaces, and other state); `off` provisions nothing. Unset behaves as `flow`. The sign-in modes never fall back to `seed` -- if `POSIT_EMAIL`/`POSIT_PASSWORD` are unset they log and the Posit AI tests skip. |
| `POSIT_EMAIL` | Both | Conditional | Posit account email for the `flow` sign-in. Required for `flow` to provision Posit AI; if unset, Posit AI tests skip. Ignored by `seed`/`off`. |
| `POSIT_PASSWORD` | Both | Conditional | Posit account password for the `flow` sign-in. Same conditional rule as `POSIT_EMAIL`. |
| `PW_SANDBOX_NO_SEED_CREDENTIALS` | Both | No | Global host-copy kill-switch. Set to `true`/`1` to block copying real credentials from the host into the sandbox: it suppresses the GitHub Copilot copy, the Posit AI `seed` copy (`PW_SANDBOX_POSITAI_AUTH=seed`), and the host-content copy in `flow-seed` (which then behaves as plain `flow`). It does **not** affect the sign-in itself, which copies nothing from the host. Default (unset): host-copy allowed. Privacy: copied tokens land inside the sandbox and persist on failed runs until you delete it -- set this on machines that aren't dedicated test accounts. |
