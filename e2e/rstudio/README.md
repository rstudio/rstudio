...
| `PW_SANDBOX_POSITAI_AUTH` | Both | No | Posit AI auth mode for the sandbox. Unset (default): if `POSIT_EMAIL`/`POSIT_PASSWORD` are set, run the OAuth device flow at start of run; otherwise leave the sandbox signed out. `seed`: copy `~/.positai/store/data.json` from the host (fails loud if the host is not signed in). `cache`: reuse a previously cached store file from `<OS cache>/rstudio-playwright-auth/positai/data.json`; on a cache miss, falls back to the device flow and re-caches. Complementary with `PW_SANDBOX_SEED_POSITAI`; see the `tests/auth.setup.ts` header for precedence detail. |
| `POSIT_EMAIL` | Both | No | Login email for the Posit AI OAuth device flow. Read when `PW_SANDBOX_POSITAI_AUTH` is unset or `cache` (and the cache misses). |
| `POSIT_PASSWORD` | Both | No | Login password for the Posit AI OAuth device flow. Same conditions as `POSIT_EMAIL`. |
