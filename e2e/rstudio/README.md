...
The suite has four Playwright projects, two of them selectable: `desktop` (default) and `server`. Both depend on `setup`, which runs first to provision AI credentials (Posit AI and GitHub Copilot) in the sandbox (see the AI authentication section below). `setup` in turn declares `auth-teardown` as its teardown, so that project runs last, after every project depending on `setup` has finished, to scrub credentials pushed to an external server. Each selectable project filters tests automatically by edition (via `PW_RSTUDIO_EDITION`, default `os`) and by OS -- `desktop` from the host's `os.platform()`, `server` always against Linux.
...
Assumes the rest of the product (gwt, the C++ session, etc.) is already built such that `npm run start` in `src/node/desktop` would launch a working IDE. The CDP-wait deadline is extended to 3 minutes on this path to accommodate the first-run webpack compile; subsequent starts are faster. Tests that exercise the doRestart() flow aren't fully supported in dev mode because the Electron relaunch spawns the same dev executable rather than a fresh CDP-enabled session.
...
The spawned server launches its rsessions through a fixture-generated wrapper script (passed as `--rsession-path`), which redirects each rsession's `HOME` into the sandbox, points `R_LIBS_USER` at the prepared package library, and unsets `XDG_CONFIG_HOME` so the copilot-language-server resolves its config dir under that `HOME`. That is what delivers the sandbox-provisioned AI credentials (and dotfile isolation) to server-mode sessions -- `rserver` otherwise builds every rsession's environment from scratch with `HOME` from the passwd db (#18348). The `@server_only` tripwire in `tests/sandbox.test.ts` asserts that `HOME` and `R_LIBS_USER` arrive with the expected values and that `XDG_CONFIG_HOME` is empty, so a break in that delivery chain fails loudly instead of quietly signing the session out.

To skip the spawn and target an external server (e.g. CI, or a remote box), set `PW_RSTUDIO_SERVER_URL`. Credentials are required only when the external server presents a login form; `PW_RSTUDIO_SERVER_PORT` overrides the port in the URL when set. Note this applies wherever the variable is set, `.env.local` included -- leaving it there quietly turns `npm run test:server-dev` into an external-server run against whatever it points at, rather than the in-tree spawn the command name suggests.

An external server's rsessions read the logged-in account's real home directory, which the harness cannot set up directly. Instead, a final auth-setup step provisions the sandbox AI credential stores **through the session** (`utils/remote-provision.ts`): it logs in, uploads each provisioned store over HTTP to the same `/upload` endpoint the Files pane posts to, moves it into place with an R console command carrying nothing but file paths, and ends the session so tests attach to a fresh rsession that reads the new stores. Keeping the bytes out of console commands is the whole design, not an incidental detail: RStudio records every submitted command in the account's history database, so a command carrying a store's contents would leave a recoverable copy of a live token there -- one this run could not clean up if the history file predated it.

A store already on the account is left alone when it holds a real sign-in; unauthenticated stubs, which sessions create on their own, are replaced. When a probe cannot be read at all, the store is left untouched and that provider's tests skip: the harness never overwrites a store it could not first inspect, since "unreadable" and "absent" are not the same thing. After the run, the `auth-teardown` project scrubs exactly the paths provisioning created -- and only those, recorded in a manifest as each one lands.

Two caveats. If the run crashes before the teardown, or the server is unreachable then, the pushed tokens remain in the remote home -- remove them by hand, and a preserved sandbox's `remote-provision-manifest.json` lists which paths. And two runs pointed at the same account will collide: they push to identical paths, so whichever finishes first scrubs the other's credentials out from under it. Run them one at a time, and use a dedicated test account for external-server AI runs (#18348).
...
- **Server**: When `PW_RSTUDIO_SERVER_URL` is unset, spawns a private in-tree `rserver-dev` per worker with sandboxed env (`--auth-none`, `--automation-agent`, isolated `HOME` / config / data dirs) and points `--rsession-path` at a generated wrapper script that carries the sandbox `HOME`, `R_LIBS_USER`, and the Copilot plaintext-store flag into each rsession, and unsets `XDG_CONFIG_HOME` there. Otherwise connects to the configured external URL. Either way, launches a headed Chromium, navigates to the server, and -- only if a login form is presented -- fills in `PW_RSTUDIO_SERVER_USER` / `_PASSWORD` before waiting for the IDE to load.
...
The Posit Assistant tests normally download the official assistant package into the sandbox `data-home`. To run them against a locally built assistant instead, set `PW_SEED_PAI` to the `pai` directory the assistant repo's `npm run deploy:rstudio` publishes to, and `globalSetup` seeds the slot it selects into `data-home/pai` as an installed version slot, from which each per-spec data home is provisioned (see Environment Variables).
...
For tests using a package outside the manifest, add it via `ensurePackages` in `beforeAll`. The helper installs into the same cache, so subsequent runs install nothing:
...
| `PW_SEED_PAI` | Both | No | Path to a locally built Posit Assistant install directory (e.g. `~/.local/share/rstudio/pai`, as produced by the assistant repo's `npm run deploy:rstudio`). The build under test is the slot the seed's `selected.json` names. `globalSetup` lays it out in the sandbox `data-home/pai` the way an install does -- `versions/<version>` with its slot manifest, named by `selected.json` -- so tests exercise that local build instead of downloading the official package (rstudio/rstudio#18658; see `fixtures/pai-seed.ts`). The seed's other slots and any legacy `bin/` are not copied. A `bin/`-only tree (`npm run deploy:rstudio:legacy`, or an older assistant checkout) is refused, since this RStudio never reads `bin/`. Each per-spec data home then gets its own copy of the layout (slots hardlink-cloned), so an install run by one spec never reaches another. Fails loud if `selected.json` is missing or unreadable, doesn't name exactly one protocol, or names a slot without a well-formed `package.json` and `protocol.json`. RStudio writes `selected.json` too: running a 2026.10+ IDE against the deployed directory and accepting an update moves the selection to the new slot, so check the seeded version `globalSetup` logs. The seeded build's version and protocol must satisfy the IDE under test, or it will be treated as needing an update. |
...
| `PW_AI_AUTH_STRICT` | Both | No | Set to `true`/`1` to make the `setup` project fail the run when a provider ends up without credentials (outcome `unavailable` or `login-failed`), instead of letting that provider's tests skip. Applies to both Posit AI and GitHub Copilot, and to the service reachability probe in `requireAiCredentials()`: an unreachable provider then fails the gated tests instead of skipping them. For runs that expect credentials to be present (e.g. CI once secrets are wired in): a broken credential source turns the run red instead of green-with-skips. Default (unset): the affected tests skip with a reason. |
