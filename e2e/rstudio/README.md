...
| `copy` | unset | Run | Only the host token store (`store/data.json`) copied here; no skills, workspaces, or other state |
...
| `PW_SANDBOX_POSITAI_AUTH` | Both | No | How the `auth.setup` project provisions Posit AI credentials: `login` (default) runs the OAuth sign-in using `POSIT_EMAIL`/`POSIT_PASSWORD`; `copy` copies only the host's token store (`~/.posit/assistant/store/data.json`), not skills, workspaces, or other state; `off` provisions nothing. Unset behaves as `login`. The sign-in mode never falls back to `copy` -- if `POSIT_EMAIL`/`POSIT_PASSWORD` are unset it logs and the Posit AI tests skip. |
