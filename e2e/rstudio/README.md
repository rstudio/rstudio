...
| `PW_SANDBOX_NO_SEED_CREDENTIALS` | Both | No | Global host-copy kill-switch. Set to `true`/`1` to block copying real credentials from the host into the sandbox: it suppresses the GitHub Copilot copy and the Posit AI `copy` mode (`PW_SANDBOX_POSITAI_AUTH=copy`). It does **not** affect the sign-in itself, which copies nothing from the host. Default (unset): host-copy allowed. Privacy: copied tokens land inside the sandbox and persist on failed runs until you delete it -- set this on machines that aren't dedicated test accounts. |
