...
GitHub Copilot follows the same shape (setting exactly one of `GH_COPILOT_USER`/`GH_COPILOT_PASSWORD` is likewise a run-failing error; "signed in locally" means the local Copilot config dir holds a signed-in `auth.db`):

| `GH_COPILOT_USER`/`GH_COPILOT_PASSWORD` | Signed in locally | `PW_SANDBOX_NO_SEED_CREDENTIALS` | Copilot Tests | Sandbox credential store |
|---|---|---|---|---|
...
| `GH_COPILOT_USER` | Both | Conditional | GitHub login (username, not email) of a dedicated Copilot test account. Setting `GH_COPILOT_USER` **and** `GH_COPILOT_PASSWORD` makes the `setup` project run the agent-driven Copilot sign-in flow (the copilot-language-server's OAuth device flow, completed in a headless browser) instead of copying the local config dir. The account must have Copilot access (e.g. Copilot Free enabled). If both are unset, the setup copies the local Copilot config dir when present, else the Copilot tests skip. Setting exactly one of the two fails the run loud. |
| `GH_COPILOT_PASSWORD` | Both | Conditional | That account's password. Paired with `GH_COPILOT_USER`; see above. |
| `GH_COPILOT_TOTP_SECRET` | Both | No | Base32 2FA secret for the Copilot test account, only needed if the account has an authenticator app enrolled. The sign-in flow generates the TOTP code from it. |
