...
**Running from a git worktree:** don't point `RSTUDIO_CPP_BUILD_OUTPUT` at the main checkout's `build/src/cpp` -- its `rdesktop-dev.conf` serves the main checkout's `src/gwt/www`, which after `ant devmode`/`ant draft` is a Super Dev Mode stub redirecting to `localhost:9876`. Without that code server, RStudio dies at startup ("code server not available"). `scripts/bootstrap-worktree.sh` builds `<worktree>/build-dev-shim` (linking the main build's `session/`, with `conf` rewritten to the worktree's own `www`) and prints the exact `PW_RSTUDIO_DEV=1 RSTUDIO_CPP_BUILD_OUTPUT=...` command to run.
