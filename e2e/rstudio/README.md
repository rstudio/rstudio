...
For unattended runs (CI) where no host state dir exists, an account `email`/`password` pair can be supplied via `POSIT_AI_EMAIL` / `POSIT_AI_PASSWORD` env vars. When both are set, `globalSetup` flips `PW_AI_SEEDED_POSITAI=1` so the matching `@ai` tests run, and `waitForChatReady()` in `actions/chat_pane.actions.ts` drives the Posit Assistant device-code OAuth flow on first chat-pane open: it extracts the `XXXX-XXXX` user_code shown in the chat pane, opens `login.posit.cloud` in a separate Playwright page, fills email + password, authorizes the device, and waits for the IDE's polling to flip the chat composer to editable. In the macOS / Windows GitHub Actions workflows these vars are sourced from 1Password (see the `Load secrets from 1Password` step in the platform reusables); the same path is used for `CONNECT_API_KEY` (the E2E Test Insights reporter). The GitHub secret needed is `OP_SERVICE_ACCOUNT_TOKEN` -- the dashboard key and account creds themselves live in 1Password and can be rotated there without re-issuing GitHub secrets.
...
| `POSIT_AI_EMAIL` / `POSIT_AI_PASSWORD` | Both | No | Posit AI account credentials. When both are set and no host state dir exists, `globalSetup` flips `PW_AI_SEEDED_POSITAI=1` and `waitForChatReady()` drives the device-code OAuth flow at `login.posit.cloud` on first chat-pane open. Sourced from 1Password in CI (the `Load secrets from 1Password` step in the platform reusable workflows); set them manually for local dev. Absent on fork PRs (no `OP_SERVICE_ACCOUNT_TOKEN`); `@ai` tests then skip. |
| `CONNECT_API_KEY` | Both | No | API key for the E2E Test Insights reporter (posts per-shard results to the Connect dashboard). Sourced from 1Password in CI via the same `Load secrets from 1Password` step. Empty on fork PRs / unconfigured repos -- the reporter still runs but its uploads warn (never fails the run). |
