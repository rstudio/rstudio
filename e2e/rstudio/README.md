...
For unattended runs (CI) where no host state dir exists, the Posit AI provider also accepts an account `email`/`password` pair via `POSIT_AI_EMAIL` / `POSIT_AI_PASSWORD` env vars. When both are set, `globalSetup` flips `PW_AI_SEEDED_POSITAI=1` without copying any host files, and the `@ai` tests drive the in-IDE sign-in flow using `getPositAiAccount()` from `utils/ai-credentials.ts`. In the macOS GitHub Actions workflow these vars are sourced from 1Password (see the `Load secrets from 1Password` step in `.github/workflows/os-test-e2e-rstudio-desktop-os-macos-26-arm64.yml`); the same path is used for `CONNECT_API_KEY` (the E2E Test Insights reporter). The GitHub secret needed for that workflow is `OP_SERVICE_ACCOUNT_TOKEN` -- the dashboard key and account creds themselves live in 1Password and can be rotated there without re-issuing GitHub secrets.
...
| `POSIT_AI_EMAIL` / `POSIT_AI_PASSWORD` | Both | No | Posit AI account credentials, consumed by `@ai`-tagged tests via `getPositAiAccount()` (`utils/ai-credentials.ts`). When both are set and no host state dir exists, `globalSetup` flips `PW_AI_SEEDED_POSITAI=1` so `@ai` tests run; they perform the in-IDE sign-in themselves. Sourced from 1Password in CI (the `Load secrets from 1Password` step in `.github/workflows/os-test-e2e-rstudio-desktop-os-macos-26-arm64.yml`); set them manually for local dev. Skipped on fork PRs (no `OP_SERVICE_ACCOUNT_TOKEN`). |
