...
| `PW_DEBUG_AUTH_STEPS` | Both | No | Set to `1`/`true` to emit the sign-in flows' step-level console output: the browser narration (`[gh-authorize]`: which page is showing, which field is being filled, button state) and the Copilot agent's raw diagnostics (`[copilot-agent]`: LSP notifications, spawn/exit, `checkStatus` polling). Off by default, so a normal run reports only the `[auth-setup]` outcome milestones (started, complete, skipped, failed) plus the final page line; the detail is opt-in for troubleshooting a flow. The output is authored text kept free of account identifiers (no username or device code), which is why it's a separate switch from `PW_DEBUG_AUTH_CAPTURE`. The Posit AI browser flow currently narrates only on failure (a consistency gap tracked separately). |
