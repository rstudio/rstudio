...
| `PW_SANDBOX_POSITAI_AUTH` | Both | No | How the `auth.setup` project provisions Posit AI credentials: `login` (default) runs the OAuth sign-in using `POSIT_EMAIL`/`POSIT_PASSWORD`; `login-copy` runs the same sign-in but first copies the host's `~/.posit/assistant` content into the sandbox, excluding the token store (host tokens are never copied -- the sign-in writes fresh ones); `copy` copies the host's whole `~/.posit/assistant` directory (tokens plus skills, workspaces, and other state); `off` provisions nothing. Unset behaves as `login`. The sign-in modes never fall back to `copy` -- if `POSIT_EMAIL`/`POSIT_PASSWORD` are unset they log and the Posit AI tests skip. |
| `POSIT_EMAIL` | Both | Conditional | Posit account email for the `login`/`login-copy` sign-in. Required for those modes to provision Posit AI; if unset, Posit AI tests skip. Ignored by `copy`/`off`. |
| `POSIT_PASSWORD` | Both | Conditional | Posit account password for the `login`/`login-copy` sign-in. Same conditional rule as `POSIT_EMAIL`. |
| `PW_SANDBOX_NO_SEED_CREDENTIALS` | Both | No | Global host-copy kill-switch. Set to `true`/`1` to block copying real credentials from the host into the sandbox: it suppresses the GitHub Copilot copy, the Posit AI `copy` mode (`PW_SANDBOX_POSITAI_AUTH=copy`), and the host-content copy in `login-copy` (which then behaves as plain `login`). It does **not** affect the sign-in itself, which copies nothing from the host. Default (unset): host-copy allowed. Privacy: copied tokens land inside the sandbox and persist on failed runs until you delete it -- set this on machines that aren't dedicated test accounts. |
