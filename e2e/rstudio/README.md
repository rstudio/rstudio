...
`HOME` / `USERPROFILE` point at a sandboxed `user-home/`. By default nothing is seeded there, so user dotfiles (`~/.Rprofile`, `~/.Renviron`, `~/.R/`, `~/.gitconfig`, `~/.ssh/`, etc.) are absent for tests. AI provider credentials are the exception, and the two providers are provisioned differently. **Posit AI** is handled by the `auth.setup` project (`tests/auth.setup.ts`), controlled by `PW_SANDBOX_POSITAI_AUTH`: by default (`copy`) it copies only the host's token store (`~/.posit/ai/auth/data.json`, or the legacy `~/.posit/assistant/store/data.json`) into the sandbox; `login` instead runs the OAuth sign-in using `POSIT_EMAIL`/`POSIT_PASSWORD` and writes a fresh token store; `off` provisions nothing. Either way the sandbox store is written to both locations, so the assistant under test finds it on either side of its store migration. Its tests gate on the on-disk token store (not an env flag) via `requireAiCredentials()`, so an unsigned-in sandbox skips cleanly. **GitHub Copilot** is still copied from the host by `globalSetup` when the config directory exists (`~/.config/github-copilot`, or `%LOCALAPPDATA%\github-copilot` on Windows); its tests gate on the `PW_AI_SEEDED_COPILOT` flag set after a successful copy. Either way, a missing credential surfaces as "skipped: no credentials" rather than a test timeout. `PW_SANDBOX_NO_SEED_CREDENTIALS=1` is a global host-copy kill-switch: it blocks the Copilot copy and Posit AI's default `copy` mode (the `login` sign-in is unaffected, since it copies nothing from the host). Note that a live sign-in against `login.posit.cloud` only happens when `PW_SANDBOX_POSITAI_AUTH=login` is set (with `POSIT_EMAIL`/`POSIT_PASSWORD` configured) -- the `setup` project runs unconditionally, even when no Posit AI test is selected. For fast local iteration while already signed in to Posit AI on the host, no configuration is needed (the default is `copy`); use `off` to skip Posit AI entirely, or `login` to exercise the live sign-in flow instead.

Quick reference, assuming each mode's own prerequisite is met (`copy` has the host already signed in; `login` has `POSIT_EMAIL`/`POSIT_PASSWORD` set):

| `PW_SANDBOX_POSITAI_AUTH` | `PW_SANDBOX_NO_SEED_CREDENTIALS` | Posit AI Tests | Sandbox token store |
|---|---|---|---|
| `copy` (default) | unset | Run | Only the host token store copied in; no skills, workspaces, or other state |
| `copy` | set | Skipped (copy suppressed) | Not copied |
| `login` | unset | Run | Nothing copied from host; a fresh token store is written here after sign-in |
...
| `PW_SANDBOX_POSITAI_AUTH` | Both | No | How the `auth.setup` project provisions Posit AI credentials: `copy` (default) copies only the host's token store (`~/.posit/ai/auth/data.json`, or the legacy `~/.posit/assistant/store/data.json`), not skills, workspaces, or other state; `login` runs the OAuth sign-in using `POSIT_EMAIL`/`POSIT_PASSWORD` instead; `off` provisions nothing. Unset behaves as `copy`. The `login` mode never falls back to `copy` -- if `POSIT_EMAIL`/`POSIT_PASSWORD` are unset it logs and the Posit AI tests skip. |
| `POSIT_EMAIL` | Both | Conditional | Posit account email for the `login` sign-in. Required for that mode to provision Posit AI; if unset, Posit AI tests skip. Ignored by `copy`/`off`. |
| `POSIT_PASSWORD` | Both | Conditional | Posit account password for the `login` sign-in. Same conditional rule as `POSIT_EMAIL`. |
| `PW_SANDBOX_NO_SEED_CREDENTIALS` | Both | No | Global host-copy kill-switch. Set to `true`/`1` to block copying real credentials from the host into the sandbox: it suppresses the GitHub Copilot copy and the Posit AI `copy` mode (the default, so an otherwise-unconfigured run skips its Posit AI tests). It does **not** affect the `login` sign-in, which copies nothing from the host. Default (unset): host-copy allowed. Privacy: copied tokens land inside the sandbox and persist on failed runs until you delete it -- set this on machines that aren't dedicated test accounts. |
