...
| `PW_SANDBOX_NO_SEED_CREDENTIALS` | Both | No | Global host-copy kill-switch. Set to `true`/`1` to block copying real credentials from the host into the sandbox: it suppresses the GitHub Copilot copy and the Posit AI `seed` copy (`PW_SANDBOX_POSITAI_AUTH=seed`). It does **not** affect Posit AI's sign-in flow mode, which copies nothing from the host. Default (unset): host-copy allowed. Privacy: copied tokens land inside the sandbox and persist on failed runs until you delete it -- set this on machines that aren't dedicated test accounts. |
