...
For unattended runs (CI) where no host state dir exists, an account `email`/`password` pair can be plumbed via `POSIT_AI_EMAIL` / `POSIT_AI_PASSWORD` env vars. `getPositAiAccount()` in `utils/ai-credentials.ts` returns them when both are set. The in-IDE sign-in click flow that consumes those credentials is not yet implemented, so when only the env vars are present (no host state dir) `globalSetup` logs the presence but leaves `PW_AI_SEEDED_POSITAI` unset and `@ai` tests continue to skip. In the macOS GitHub Actions workflow these vars are sourced from 1Password (see the `Load secrets from 1Password` step in `.github/workflows/os-test-e2e-rstudio-desktop-os-macos-26-arm64.yml`); the same path is used for `CONNECT_API_KEY` (the E2E Test Insights reporter). The GitHub secret needed for that workflow is `OP_SERVICE_ACCOUNT_TOKEN` -- the dashboard key and account creds themselves live in 1Password and can be rotated there without re-issuing GitHub secrets.
...
| `POSIT_AI_EMAIL` / `POSIT_AI_PASSWORD` | Both | No | Posit AI account credentials reachable via `getPositAiAccount()` (`utils/ai-credentials.ts`). Plumbed for a follow-up sign-in flow; today `@ai` tests still skip when only these env vars are present (no host state dir) because the in-IDE sign-in click flow is not yet wired. Sourced from 1Password in CI (the `Load secrets from 1Password` step in `.github/workflows/os-test-e2e-rstudio-desktop-os-macos-26-arm64.yml`); set them manually for local dev. Absent on fork PRs (no `OP_SERVICE_ACCOUNT_TOKEN`). |
