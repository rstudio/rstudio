...
{
   return !rstudiocore::system::isHiddenFile(fileInfo);
}
...
public:
   ...
   RegistrationCommand(const rstudiocore::FilePath& filePath,
                       bool recursive,
                       const boost::function<bool(const FileInfo&)>& filter,
                       const Callbacks& callbacks)
      : type_(Register),
...
typedef rstudiocore::thread::ThreadsafeQueue<RegistrationCommand>
                                                      RegistrationCommandQueue;
RegistrationCommandQueue& registrationCommandQueue()
{
   static rstudiocore::thread::ThreadsafeQueue<RegistrationCommand> instance;
   return instance;
}

typedef rstudiocore::thread::ThreadsafeQueue<boost::function<void()> > CallbackQueue;
CallbackQueue& callbackQueue()
{
   static rstudiocore::thread::ThreadsafeQueue<boost::function<void()> > instance;
   return instance;
}
...
void initialize()
{
   ...
   rstudiocore::thread::safeLaunchThread(fileMonitorThreadMain, &s_fileMonitorThread);
}
