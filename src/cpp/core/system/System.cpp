...
{
   rstudiocore::log::logError(error, loggedFromLocation);
   return EXIT_FAILURE;
}
...
{
   rstudiocore::log::logErrorMessage(errMsg, loggedFromLocation);
   return EXIT_FAILURE;
}
...
{
   std::string uuid = rstudiocore::system::generateUuid(false);
   return rstudiocore::hash::crc32HexHash(uuid);
}
