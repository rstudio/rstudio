...
// NOTE: this function is duplicated between here and rstudiocore::system
...
{
   rstudiocore::system::closeStdFileDescriptors();
   rstudiocore::system::attachStdFileDescriptorsToDevNull();
...
Error processInfo(const std::string& process, std::vector<ProcessInfo>* pInfo)
{
   ...
   try
   {
      ...
      while ( (pDirent = ::readdir(pDir)) )
      {
         ...
         // read the cmdline
         std::string cmdline;
         Error error = rstudiocore::readStringFromFile(cmdlineFile, &cmdline);
         if (error)
         {
            LOG_ERROR(error);
            continue;
         ...
         if (FilePath(cmdline).filename() == process)
         {
            ...
            // get the username
            rstudiocore::system::user::User user;
            Error error = rstudiocore::system::user::userFromId(st.st_uid, &user);
            if (error)
            {
               LOG_ERROR(error);
               continue;
...
Error restrictCoreDumps()
{
   ...
   Error error = setResourceLimit(rstudiocore::system::CoreLimit, 0);
   if (error)
      return error;
...
void printCoreDumpable(const std::string& context)
{
   ...
   Error error = getResourceLimit(rstudiocore::system::CoreLimit,
                         ...
                         PidType* pProcessId)
{
   ...
   else if (pid == 0)
   {
      ...
      Error error = rstudiocore::system::clearSignalMask();
      if (error)
      {
         LOG_ERROR(error);
         ::exit(EXIT_FAILURE);
      ...
      rstudiocore::system::setenv(&env, "USER", user.username);
      rstudiocore::system::setenv(&env, "LOGNAME", user.username);
      rstudiocore::system::setenv(&env, "HOME", user.homeDirectory);
      copyEnvironmentVar("SHELL", &env);

      // add custom environment vars (overriding as necessary)
      for (rstudiocore::system::Options::const_iterator it = config.environment.begin();
           it != config.environment.end();
           ++it)
      {
         rstudiocore::system::setenv(&env, it->first, it->second);
      }
      ...
      for(rstudiocore::system::Options::const_iterator it = env.begin();
           it != env.end();
           ++it)
      {
      ...
      rstudiocore::system::ProcessArgs* pEnvironment = new rstudiocore::system::ProcessArgs(
                                                                       envVars);

      // build process args
      std::vector<std::string> argVector;
      argVector.push_back(path);
      for (rstudiocore::system::Options::const_iterator it = config.args.begin();
           it != config.args.end();
           ++it)
      {
      ...
      rstudiocore::system::ProcessArgs* pProcessArgs = new rstudiocore::system::ProcessArgs(
                                                                  argVector);
