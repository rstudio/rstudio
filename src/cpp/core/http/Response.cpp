...
{
   std::string content;
   Error error = rstudiocore::readStringFromFile(filePath, &content);
   if (error)
      return error;
                                ...
                                const Request& request)
{
   ...
   std::string contents;
   Error error = rstudiocore::readStringFromFile(filePath, &contents);
   if (error)
   {
      setError(error);
      return;
...
{
   return rstudiocore::hash::crc32Hash(content);
}   
