...
public:
   ...
   // Check both locking schemes when processes may use different defaults.
   // As with advisory isLocked(), only use this to inspect other processes'
   // locks: probing an advisory lock held by this process can release it.
   static bool isLockedByAnyType(const FilePath& lockFilePath);
