...
struct Handle
{
   ...
   explicit Handle(void* pData)
      : id(rstudiocore::system::generateUuid()),
        pData(pData)
   {
   }
...
struct Callbacks
{
   ...
   boost::function<void(const rstudiocore::Error&)> onRegistrationError;
   ...
   boost::function<void(const rstudiocore::Error&)> onMonitoringError;
...
void registerMonitor(const rstudiocore::FilePath& filePath,
                     bool recursive,
