...
public:
 ...
 public:
   ...
   bool empty() const { 
      ...
      return scratchPath_.isEmpty() && empty;
   }
                 ...
                 bool projectSharingEnabled) const
   {
      ...
      Error error = storage_->isValid(&propertiesValid);

      if (error)
      {
         LOG_ERROR_MESSAGE("Unable to validate properties storage, [ "+ error.getSummary()  +" ]");
      }
