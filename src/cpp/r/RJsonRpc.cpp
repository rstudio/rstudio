...
Error setJsonResult(SEXP resultSEXP, rstudiocore::json::JsonRpcResponse* pResponse)
{   
   // get the result
   rstudiocore::json::Value resultValue ;
                   ...
                   sexp::Protect* pProtect)
{
   ...
   // add params
   const rstudiocore::json::Array& params = request.params;
   for (rstudiocore::json::Array::size_type i=0; i<params.size(); i++)
      rFunction.addParam(params[i]);
   
   // add kwparams
   const rstudiocore::json::Object& kwparams = request.kwparams;
   for (rstudiocore::json::Object::const_iterator 
        it = kwparams.begin();
        it != kwparams.end();
        ++it)
   {
...
Error getRpcMethods(rstudiocore::json::JsonRpcMethods* pMethods)
{
