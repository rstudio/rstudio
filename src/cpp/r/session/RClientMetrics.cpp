...
const char * const kIsRetina = "r.session.client_metrics.is-retina";
...
RClientMetrics get()
{
   ...
   metrics.isRetina = graphics::device::isRetina();
   return metrics;
}
...
void set(const RClientMetrics& metrics)
{
   ...
   if (metrics.graphicsWidth > 0 && metrics.graphicsHeight > 0)
   {
      ...
      // set device size
      graphics::device::setSize(width, height, metrics.isRetina);
   }
}
...
void save(Settings* pSettings)
{
   ...
   pSettings->set(kIsRetina, metrics.isRetina);
   pSettings->endUpdate();
}
...
void restore(const Settings& settings)
{
   ...
   metrics.isRetina = settings.getInt(kIsRetina,
                                      graphics::device::kDefaultIsRetina);
