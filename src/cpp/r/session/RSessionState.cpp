...
Error saveEnvironmentVars(const FilePath& envFile)
{
   ...
   if (error)
      return error;
   rstudiocore::Settings envSettings;
   ...
   BOOST_FOREACH(const rstudiocore::system::Option& var, env)
   {
                                       ...
                                       boost::function<void(const char*)>())
{
   ...
   rstudiocore::log::logError(serializationError, location);
...
bool saveMinimal(const rstudiocore::FilePath& statePath,
...
bool getBoolSetting(const rstudiocore::FilePath& statePath,
...
bool rProfileOnRestore(const rstudiocore::FilePath& statePath)
{
   return getBoolSetting(statePath, kRProfileOnRestore, true);
}

bool packratModeEnabled(const rstudiocore::FilePath& statePath)
{
