...
QString Options::portNumber() const
{
   ...
   if (portNumber_.length() == 0)
   {
      ...
      int base = std::abs(rstudiocore::random::uniformRandomInteger<int>());
      ...
      rstudiocore::system::setenv("RS_LOCAL_PEER", localPeer_);
   }
...
FilePath Options::executablePath() const
{
   ...
   {
      Error error = rstudiocore::system::executablePath(QApplication::arguments().at(0).toUtf8(),
                                                 &executablePath_);
      if (error)
         LOG_ERROR(error);
   }
...
FilePath Options::supportingFilePath() const
{
   if (supportingFilePath_.empty())
   {
      ...
      rstudiocore::system::installPath("..",
...
rstudiocore::FilePath Options::scratchTempDir(rstudiocore::FilePath defaultPath)
{
   rstudiocore::FilePath dir(scratchPath.toUtf8().constData());

   if (!dir.empty() && dir.exists())
   {
      dir = dir.childPath("tmp");
      rstudiocore::Error error = dir.ensureDirectory();
      if (!error)
         return dir;
   }
   return defaultPath;
...
{
   rstudiocore::FilePath temp = scratchTempDir(rstudiocore::FilePath());
   if (!temp.empty())
      temp.removeIfExists();
}
