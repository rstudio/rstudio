...
# Test rs_getThemes ================================================================================
test_that("rs_getThemes works correctly with local installs (relies on .rs.addTheme)", {
   addedThemes <- list()
   .rs.enumerate(themes, function(themeName, themeDesc) {
         tryCatch({
         .rs.addTheme(
            file.path(
               inputFileLocation,
               "rsthemes",
               paste0(themeDesc$fileName, ".rstheme")),
            apply = FALSE,
            force = FALSE,
            globally = FALSE)
         
         addedThemes[[themeName]] <<- file.path(localInstallDir, paste0(themeDesc$fileName, ".rstheme"))
      },
      error = function(e) {
   ...
   {
      infoStr <- paste("Theme:", themeName)
      expect_true(themeName %in% themeList, info = infoStr)
      expect_equal(themeList[[themeName]]$fileName, themeLocation, info = infoStr)
      expect_equal(themeList[[themeName]]$isDark, themes[[themeName]]$isDark, info = infoStr)
      file.remove(themeLocation)
   })
})
