...
void httpServerAddHandlers()
{
   ...
   uri_handlers::addBlocking("/progress",
                               ...
                               rstudiocore::text::handleSecureTemplateRequest,
...
int main(int argc, char * const argv[]) 
{
   try
   {
      ...
      initializeSystemLog(kProgramIdentity, rstudiocore::system::kLogLevelWarning);

      // ignore SIGPIPE (don't log error because we should never call
      // syslog prior to daemonizing)
      rstudiocore::system::ignoreSignal(rstudiocore::system::SigPipe);
      ...
      {
         Error error = rstudiocore::system::daemonize();
         if (error)
            return rstudiocore::system::exitFailure(error, ERROR_LOCATION);

         error = rstudiocore::system::ignoreTerminalSignals();
         if (error)
            return rstudiocore::system::exitFailure(error, ERROR_LOCATION);

         // set file creation mask to 022 (might have inherted 0 from init)
         setUMask(rstudiocore::system::OthersNoWriteMask);
      }
      ...
      {
         Error error = setResourceLimit(rstudiocore::system::FilesLimit, 4096);
         if (error)
            return rstudiocore::system::exitFailure(error, ERROR_LOCATION);
      }
      ...
      rstudiocore::system::crypto::initialize();
      ...
      rstudiocore::system::addLogWriter(
      ...
      {
         // drop root priv
         Error error = rstudiocore::system::temporarilyDropPriv(runAsUser);
         if (error)
            return rstudiocore::system::exitFailure(error, ERROR_LOCATION);
      }
