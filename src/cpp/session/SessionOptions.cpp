...
rstudiocore::ProgramStatus Options::read(int argc, char * const argv[])
{
   using namespace boost::program_options ;
   
   // get the shared secret
   monitorSharedSecret_ = rstudiocore::system::getenv(kMonitorSharedSecretEnvVar);
   rstudiocore::system::unsetenv(kMonitorSharedSecretEnvVar);

   // compute the resource path
   FilePath resourcePath;
   Error error = rstudiocore::system::installPath("..", argv[0], &resourcePath);
   if (error)
   {
   ...
   std::string currentUsername = rstudiocore::system::username();
   ...
   rstudiocore::program_options::OptionsDescription optionsDesc("rsession",
                                                         configFile);
   ...
   if (verifyInstallation_)
   {
      ...
      if (programMode_ == kSessionProgramModeServer)
      {
         ...
         rstudiocore::system::setenv("R_USER", verifyInstallationHomeDir_);
      }
   }
   ...
   if (standalone())
      rstudiocore::system::setenv("HOME", userHomePath_);
   ...
   secret_ = rstudiocore::system::getenv("RS_SHARED_SECRET");
   ...
   //rstudiocore::system::unsetenv("RS_SHARED_SECRET");

   // initial working dir override
   initialWorkingDirOverride_ = rstudiocore::system::getenv(kRStudioInitialWorkingDir);
   rstudiocore::system::unsetenv(kRStudioInitialWorkingDir);

   // initial environment file override
   initialEnvironmentFileOverride_ = rstudiocore::system::getenv(kRStudioInitialEnvironment);
   rstudiocore::system::unsetenv(kRStudioInitialEnvironment);

   // initial project
   initialProjectPath_ = rstudiocore::system::getenv(kRStudioInitialProject);
   rstudiocore::system::unsetenv(kRStudioInitialProject);

   // limit rpc client uid
   limitRpcClientUid_ = -1;
   std::string limitUid = rstudiocore::system::getenv(kRStudioLimitRpcClientUid);
   if (!limitUid.empty())
   {
      limitRpcClientUid_ = rstudiocore::safe_convert::stringTo<int>(limitUid, -1);
      rstudiocore::system::unsetenv(kRStudioLimitRpcClientUid);
   }
