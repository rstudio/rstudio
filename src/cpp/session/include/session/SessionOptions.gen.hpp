                ...
                std::string* pSessionPortRange)
{
   ...
   pExternal->add_options()
      ...
      "Specifies the path to Copilot Language Server JavaScript distribution.")
      ("external-node-path",
   ...
   pTrust->add_options()
      ...
      "When enabled, projects are treated as untrusted by default: users are prompted to trust each project when it is opened, even if it contains no auto-executing files. Projects that have not been explicitly trusted run in restricted mode. Setting this option implies project-trust-dialogs, unless that option has been explicitly disabled.")
