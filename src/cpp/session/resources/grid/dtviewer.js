...
(function () {
   ...
   var createNumericFilterUI = function (idx, col, onDismiss) {
      ...
      invokeFilterPopup(ele, function (popup) {
         ...
         for (var i = 0; i < col.col_breaks.length; i++) {
            if (Math.abs(col.col_breaks[i] - min) < Math.abs(col.col_breaks[binStart] -
                  min)) {
               binStart = i;
            }
            if (i === 0)
               continue;
            if (Math.abs(col.col_breaks[i] - max) < Math.abs(col.col_breaks[binEnd] -
                  max)) {
               binEnd = i - 1;
            }
         }
   ...
   var createTextFilterBox = function (ele, idx, col, onDismiss) {
      ...
      var updateView = debounce(function () {
         table.columns(idx).search("character|" + encodeURIComponent(input.value))
         .draw();
      }, 200);
   ...
   var createColumnTypesUI = function (th, idx, col) {
      ...
      val.textContent = "(" + (col.col_type_assigned ? col.col_type_assigned : col
         .col_type_r) + ")";
   ...
   var initDataTable = function (resCols, data) {
         ...
         dataTableColumns = resCols.length > 0 ? cols.map(function (e, idx) {
            ...
            return {
               ...
               "data": function (row, type, set, meta) {
                  return (meta.col === 0 ? meta.row : (data ? data[meta.col - 1][meta
                     .row
                  ] : null));
               },
               "width": "4em",
      ...
      $("#rsGridData").dataTable({
         ...
         "fnInfoCallback": !statusTextOverride ? null : function (oSettings, iStart,
   ...
   var addGlobalResizeHandlers = function() {
     ...
     if (!!document.resizeHandlersInit)
       return;
   ...
   window.setColumnDefinitionsUIVisible = function (visible, onColOpen, onColDismiss) {
      ...
      return setHeaderUIVisible(visible, setColumnDefinitionsUIVisiblePerColumn,
         hideColumnTypeUI);
   };
   ...
   window.nextColumnPage = function () {
      ...
      var newOffset = Math.max(0, Math.min(cols.length - 1 - maxColumns, columnOffset +
         maxColumns));
   ...
   var parsedLocation = parseLocationUrl();
   var dataMode = parsedLocation && parsedLocation.dataSource ? parsedLocation.dataSource :
      "server";
