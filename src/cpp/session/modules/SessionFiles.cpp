...
rstudiocore::Error stat(const json::JsonRpcRequest& request,
...
rstudiocore::Error isTextFile(const json::JsonRpcRequest& request,
                       json::JsonRpcResponse* pResponse)
{
   std::string path;
   Error error = json::readParams(request.params, &path);
...
rstudiocore::Error getFileContents(const json::JsonRpcRequest& request,
                            json::JsonRpcResponse* pResponse)
{
...
Error listFiles(const json::JsonRpcRequest& request, json::JsonRpcResponse* pResponse)
{
   ...
   rstudiocore::json::Array jsonFiles;
   if (monitor)
   {
...
// IN: String path
rstudiocore::Error createFolder(const rstudiocore::json::JsonRpcRequest& request,
                         json::JsonRpcResponse* pResponse)
{
   std::string path;
...
rstudiocore::Error deleteFile(const FilePath& filePath)
{
   if (session::options().programMode() == kSessionProgramModeDesktop)
   {
      Error error = rstudiocore::system::recycle_bin::sendTo(filePath);
      if (error)
      {
         LOG_ERROR(error);
...
rstudiocore::Error deleteFiles(const rstudiocore::json::JsonRpcRequest& request,
                        json::JsonRpcResponse* pResponse)
{
   json::Array files;
...
Error copyFile(const rstudiocore::json::JsonRpcRequest& request,
               json::JsonRpcResponse* pResponse)
{
   // read params
...
Error moveFiles(const rstudiocore::json::JsonRpcRequest& request,
                json::JsonRpcResponse* pResponse)
{
   json::Array files;
...
rstudiocore::Error renameFile(const rstudiocore::json::JsonRpcRequest& request,
                       json::JsonRpcResponse* pResponse)
{
   // read params
...
Error completeUpload(const rstudiocore::json::JsonRpcRequest& request,
                     json::JsonRpcResponse* pResponse)
{
   // read params
   json::Object token;
                             ...
                             http::Response* pResponse) 
{
   ...
   Error saveError = rstudiocore::writeStringToFile(tempFilePath, file.contents);
   if (saveError)
   {
      LOG_ERROR(saveError);
