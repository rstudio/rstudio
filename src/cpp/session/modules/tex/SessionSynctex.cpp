...
json::Value toJson(const FilePath& pdfFile,
                   const rstudiocore::tex::PdfLocation& pdfLoc,
                   bool fromClick)
{
   if (!pdfLoc.empty())
   {
...
json::Value toJson(const rstudiocore::tex::SourceLocation& srcLoc)
{
   if (!srcLoc.empty())
   {
      json::Object srcJson;
...
void applyForwardConcordance(const FilePath& mainFile,
                             rstudiocore::tex::SourceLocation* pLoc)
{
   ...
   if (!texLine.empty())
   {
      *pLoc = rstudiocore::tex::SourceLocation(texLine.filePath(),
                                        texLine.line(),
                                        pLoc->column());
   }
}
...
json::Object sourceLocationAsJson(const rstudiocore::tex::SourceLocation& srcLoc,
                                  bool fromClick)
{
...
void applyInverseConcordance(rstudiocore::tex::SourceLocation* pLoc)
{
    // try to read concordance
   using namespace tex::rnw_concordance;
   Concordances concordances;
   ...
   if (!rnwLine.empty())
   {
      *pLoc = rstudiocore::tex::SourceLocation(rnwLine.filePath(),
                                        rnwLine.line(),
                                        pLoc->column());
   }
}
                                 ...
                                 json::JsonRpcResponse* pResponse)
{
   ...
   rstudiocore::tex::SourceLocation srcLoc(srcPath, line, column);
                           ...
                           json::JsonRpcResponse* pResponse)
{
   ...
   rstudiocore::tex::Synctex synctex;
   if (synctex.parse(pdfPath))
   {
      if (!fromClick)
      {
         ...
         rstudiocore::tex::PdfLocation contLoc = synctex.topOfPageContent(page);
         x = std::max((float)x, contLoc.x());
         y = std::max((float)y, contLoc.y());
      }

      rstudiocore::tex::PdfLocation pdfLocation(page, x, y, width, height);

      rstudiocore::tex::SourceLocation srcLoc = synctex.inverseSearch(pdfLocation);
      applyInverseConcordance(&srcLoc);
                       ...
                       const module_context::PostbackHandlerContinuation& cont)
{
   ...
   rstudiocore::tex::SourceLocation srcLoc(FilePath(sourceFile), line, 1);
   applyInverseConcordance(&srcLoc);
                    ...
                    json::Value* pPdfLocation)
{
   ...
   {
      rstudiocore::tex::SourceLocation srcLoc(inputFile, line, column);
      applyForwardConcordance(rootFile, &srcLoc);

      rstudiocore::tex::PdfLocation pdfLoc = synctex.forwardSearch(srcLoc);
