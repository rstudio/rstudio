                         ...
                         const FilePath& baseDir)
{
   ...
   outputFile_ = baseDir.complete(rstudiocore::tex::normalizeSynctexName(sections[1]));
   inputFile_ = baseDir.complete(rstudiocore::tex::normalizeSynctexName(sections[2]));
                            ...
                            const boost::smatch& what)
{
   ...
   for (unsigned int i = what.size()-1; i > 0; i--)
   {
      if (what[i].matched)
      {
         int inputLine = rstudiocore::safe_convert::stringTo<int>(what[i], 1);
...
rstudiocore::tex::LogEntry Concordances::fixup(const rstudiocore::tex::LogEntry &entry,
                                        bool *pSuccess) const
{
   ...
   if (!mapped.empty())
   {
      ...
      return rstudiocore::tex::LogEntry(entry.logFilePath(),
                                 entry.logLine(),
                                 entry.type(),
                                 mapped.filePath(),
                                 mapped.line(),
...
Error readIfExists(const rstudiocore::FilePath& srcFile, Concordances* pConcordances)
{
   ...
   // read the file
   std::string contents;
   Error error = rstudiocore::readStringFromFile(concordanceFile,
                                          &contents,
