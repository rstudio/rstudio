...
FilePath historyEntriesPath(const rstudiocore::FilePath& serializationPath)
{
   return serializationPath.complete("history_entries");
}

FilePath currentIndexPath(const rstudiocore::FilePath& serializationPath)
{
...
void ViewerHistory::saveTo(const rstudiocore::FilePath& serializationPath) const
{
   ...
   error = rstudiocore::writeStringToFile(currentIndexPath(serializationPath),
                                   currentIndex);
   if (error)
   {
      LOG_ERROR(error);
...
void ViewerHistory::restoreFrom(const rstudiocore::FilePath& serializationPath)
{
   ...
   Error error = rstudiocore::readStringFromFile(indexPath, &currentIndex);
   if (error)
   {
      LOG_ERROR(error);
      return;
...
rstudiocore::Error ViewerHistoryEntry::copy(
             const rstudiocore::FilePath& sourceDir,
