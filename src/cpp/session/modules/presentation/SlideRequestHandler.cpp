                 ...
                 ErrorResponse* pErrorResponse)
{
   ...
   // run knit
   error = rstudiocore::system::runProgram(
            rstudiocore::string_utils::utf8ToSystem(rProgramPath.absolutePath()),
            args,
            "",
            options,
            &result);
   ...
   else if (result.exitStatus != EXIT_SUCCESS)
   {
      ...
      if (!mdPath.exists())
      {
         Error error = rstudiocore::writeStringToFile(mdPath,
                                               mdPath.stem() +
                                               "\n=======================\n");
         if (error)
            LOG_ERROR(error);
      ...
      Error error = rstudiocore::appendToFile(mdPath, ostr.str());
      if (error)
         LOG_ERROR(error);
...
std::string userSlidesCss(const SlideDeck& slideDeck)
{
   ...
   if (cssPath.exists())
   {
      Error error = rstudiocore::readStringFromFile(cssPath, &userSlidesCss);
      if (error)
         LOG_ERROR(error);
   }
                        ...
                        http::Response* pResponse)
{
       ...
       targetFile.lastWriteTime() != s_cache.file.lastWriteTime())
   {
      ...
      Error error = rstudiocore::readStringFromFile(targetFile, &(s_cache.contents));
      if (error)
      {
         pResponse->setError(error);
         return;
