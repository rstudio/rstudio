...
public:
   ...
   std::string readOutput() const
   {
      ...
      std::string output;
      Error error = rstudiocore::readStringFromFile(outputFile_, &output);
      if (error)
         LOG_ERROR(error);
...
private:
                        ...
                        const std::string& encoding)
   {
      if (exitStatus == EXIT_SUCCESS)
      {
         ...
         if (knitrOutputFile_.empty())
         {
            ...
            Error error = rstudiocore::readStringFromFile(outputPathTempFile,
                                                   &outputFile);
            if (error)
            {
               terminateWithError(error);
                             ...
                             const std::string& encoding)
   {
      ...
      else
      {
         ...
         error = rstudiocore::writeStringToFile(outputFile, content);
         if (error)
            terminateWithError(error);
         else
                          ...
                          std::string* pPreviewTemplate)
{
   ...
   return rstudiocore::readStringFromFile(htmlPreviewFile, pPreviewTemplate);
}
                                  ...
                                  http::Response* pResponse)
{
   try
   {
      ...
      error = rstudiocore::writeStringToFile(s_pCurrentPreview_->htmlPreviewFile(),
                                      previewHtml);
      if (error)
      {
         pResponse->setError(error);
...
rstudiocore::json::Object capabilitiesAsJson()
{
   ...
   else
   {
      ...
      if (error)
         LOG_ERROR(error);
      else if (rstudiocore::json::isType<rstudiocore::json::Object>(valJson))
