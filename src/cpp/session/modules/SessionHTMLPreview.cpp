...
Error createNotebook(const json::JsonRpcRequest& request,
                     json::JsonRpcResponse* pResponse)
{
   static const std::string MAGIC_GUID("12861c30b10411e1afa60800200c9a66");
   ...
   if (realPath.exists())
   {
      ...
      if (eod == std::search(std::istreambuf_iterator<char>(*pStr),
                             ...
                             MAGIC_GUID.end()))
      {
         // TODO: Return a specific error indicating that the file we wanted
   ...
   prefix = "<!-- Automatically generated by RStudio [" + MAGIC_GUID +
         "] -->\n" + prefix;
