...
Error FilesListingMonitor::start(const FilePath& filePath, json::Array* pJsonFiles)
{
   ...
   cb.onRegistrationError =  boost::bind(rstudiocore::log::logError, _1, ERROR_LOCATION);
   cb.onFilesChanged = boost::bind(module_context::enqueFileChangedEvents, filePath, _1);
   cb.onMonitoringError = boost::bind(rstudiocore::log::logError, _1, ERROR_LOCATION);
   cb.onUnregistered = boost::bind(&FilesListingMonitor::onUnregistered, this, _1);
   rstudiocore::system::file_monitor::registerMonitor(filePath,
                                               false,
...
void FilesListingMonitor::stop()
{
   ...
   {
      rstudiocore::system::file_monitor::unregisterMonitor(currentHandle_);
      currentHandle_ = rstudiocore::system::file_monitor::Handle();
   }
}
...
//     is taken using rstudiocore::scanFiles). When propagated to the client this
//     results in symlinked directories appearing as documents and not
//     being traversable in the files pane
//
//   - We could fix this by changing the behavior of rstudiocore::scanFiles and/or
...
void FilesListingMonitor::onRegistered(rstudiocore::system::file_monitor::Handle handle,
                                       ...
                                       const tree<rstudiocore::FileInfo>& files)
{
   ...
   std::vector<rstudiocore::system::FileChangeEvent> events;
   rstudiocore::system::collectFileChangeEvents(prevFiles.begin(),
                                         prevFiles.end(),
                                         currFiles.begin(),
                                         currFiles.end(),
                                         module_context::fileListingFilter,
...
void FilesListingMonitor::onUnregistered(rstudiocore::system::file_monitor::Handle handle)
{
                                     ...
                                     json::Array* pJsonFiles)
{
   // enumerate the files
   pFiles->clear();
   rstudiocore::Error error = rootPath.children(pFiles) ;
   if (error)
      return error;
   ...
   std::sort(pFiles->begin(), pFiles->end(), rstudiocore::compareAbsolutePathNoCase);

   // produce json listing
   BOOST_FOREACH( rstudiocore::FilePath& filePath, *pFiles)
   {
      // files which may have been deleted after the listing or which
      // are not end-user visible
      if (filePath.exists() && module_context::fileListingFilter(rstudiocore::FileInfo(filePath)))
      {
         rstudiocore::json::Object fileObject = module_context::createFileSystemItem(filePath);
