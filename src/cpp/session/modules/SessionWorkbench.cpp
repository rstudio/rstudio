...
void ammendShellPaths(T* pTarget)
{
   ...
   if (!gitBinDir.empty())
      rstudiocore::system::addToPath(pTarget, gitBinDir);

   // non-path svn bin dir
   std::string svnBinDir = svn::nonPathSvnBinDir();
   if (!svnBinDir.empty())
      rstudiocore::system::addToPath(pTarget, svnBinDir);

   // msys_ssh path
   rstudiocore::system::addToPath(pTarget,
                      ...
                      json::JsonRpcResponse* pResponse)
{
   ...
   rstudiocore::system::addToPath(&childEnv,
                           session::options().msysSshPath().absolutePath());
                      ...
                      const module_context::PostbackHandlerContinuation& cont)
{
   ...
   Error error = rstudiocore::readStringFromFile(filePath, &fileContents);
   if (error)
   {
      LOG_ERROR(error);
      cont(EXIT_FAILURE, "");
   ...
   error = rstudiocore::writeStringToFile(filePath, editedFileContents);
   if (error)
   {
      LOG_ERROR(error);
      cont(EXIT_FAILURE, "");
                       ...
                       json::JsonRpcResponse* pResponse)
{
   ...
   rstudiocore::system::Options shellEnv;
   rstudiocore::system::environment(&shellEnv);

   // set dumb terminal
   rstudiocore::system::setenv(&shellEnv, "TERM", "dumb");

   // set prompt
   std::string path = module_context::createAliasedPath(
                                 module_context::safeCurrentPath());
   std::string prompt = (path.length() > 30) ? "\\W$ " : "\\w$ ";
   rstudiocore::system::setenv(&shellEnv, "PS1", prompt);

   // disable screen oriented facillites
   rstudiocore::system::unsetenv(&shellEnv, "EDITOR");
   rstudiocore::system::unsetenv(&shellEnv, "VISUAL");
   rstudiocore::system::setenv(&shellEnv, "PAGER", "/bin/cat");

   rstudiocore::system::setenv(&shellEnv, "GIT_EDITOR", s_editFileCommand);
   rstudiocore::system::setenv(&shellEnv, "SVN_EDITOR", s_editFileCommand);
   ...
   rstudiocore::shell_utils::ShellCommand bashCommand("/bin/bash");
   bashCommand << "--norc";
