                            ...
                            const std::string& nextLineContents)
{
   ...
   BOOST_FOREACH(const FilePath& child, children)
   {
      if (isRSourceFile(child))
      {
         ...
         Error error = rstudiocore::readStringVectorFromFile(child, &lines, false);
         if (error)
         {
            LOG_ERROR(error);
            continue;
                                       ...
                                       const std::string& output)
{
   ...
   for (; iter != end; iter++)
   {
      ...
      int diagLine = rstudiocore::safe_convert::stringTo<int>(match[4], -1);
      if (diagLine != -1)
      {
         ...
         if (!rSrcFile.empty())
         {
            ...
            CompileError err(CompileError::Error,
                             rSrcFile,
                             rstudiocore::safe_convert::stringTo<int>(line, 1),
                             rstudiocore::safe_convert::stringTo<int>(column, 1),
                             message,
                             false);
                                         ...
                                         const std::string& output)
{
   ...
   for (; iter != end; iter++)
   {
      ...
      FilePath realPath;
      Error error = rstudiocore::system::realPath(filePath, &realPath);
      if (error)
         LOG_ERROR(error);
      else
         filePath = realPath;
