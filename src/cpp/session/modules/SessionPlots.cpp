...
{   
   ...
   {
      return Error(rstudiocore::json::errc::ParamInvalid, ERROR_LOCATION);
   }
                            ...
                            json::JsonRpcResponse* pResponse)
{
   ...
   error = rstudiocore::writeStringToFile(sourceFilePath, html);
   if (error)
      return error;
...
#ifdef __APPLE__
   std::string display = rstudiocore::system::getenv("DISPLAY");
