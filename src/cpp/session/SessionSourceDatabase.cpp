...
bool isIntendedAsReadOnly(const std::string& contents,
                          std::string* pAlternative)
{
   boost::smatch match;
   
   // check for roxygen token specifically
   boost::regex reRoxygen(
            "% Generated by roxygen2: do not edit by hand\n"
            "% Please edit documentation in ([^\n]+)");
   
   if (regex_utils::search(contents, match, reRoxygen))
   {
      *pAlternative = match[1];
      return true;
   }
...
void SourceDocument::writeToJson(json::Object* pDocJson, bool includeContents) const
{
   ...
   jsonDoc["read_only"] = isIntendedAsReadOnly(contents(), &alternative);
   jsonDoc["read_only_alternative"] = alternative;
}
