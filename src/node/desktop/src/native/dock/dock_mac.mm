...
// 'dock.node.d.ts', to ensure that the JavaScript type definitions are
...
void setDockLabelImpl(NSString* label)
{
   if ([label length] == 0)
      [[MacDockTileView sharedTileView] setLabel: nil];
   else
   {
      [[MacDockTileView sharedTileView] setLabel: [label lastPathComponent]];
   }
}
