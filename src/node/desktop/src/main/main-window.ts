...
export class MainWindow extends GwtWindow {
  ...
  onLoadFinished(ok: boolean): void {
    if (ok) {
      ...
      if (reloadCount === 0) {
        ...
        this.loadUrl(getRendererUrl('src/ui/loading/loading.html'), false).catch(logger().logError);
  ...
  onLoadFailed(): void {
    ...
    this.loadUrl(getRendererUrl('src/ui/connect/connect.html')).catch(logger().logError);
  }
