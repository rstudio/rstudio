...
import { getRendererUrl, getPreloadPath } from './vite-urls';
import { createLocalUrlChecker } from './whats-new-utils';

const WHATS_NEW_HOST_PAGE = 'src/ui/whats-new/whats-new-host.html';
const WHATS_NEW_PRELOAD_FILE = 'whats-new-preload.js';
...
export function showWhatsNewWindow(options: WhatsNewWindowOptions): BrowserWindow {
  ...
  const win = new BrowserWindow({
    ...
    webPreferences: {
      ...
      sandbox: true,
      preload: getPreloadPath(WHATS_NEW_PRELOAD_FILE),
  ...
  const hostUrl = getRendererUrl(WHATS_NEW_HOST_PAGE);
  const isLocalUrl = createLocalUrlChecker(hostUrl, options.releaseSlug);
  ...
  const url = `${hostUrl}${separator}${params.toString()}`;
