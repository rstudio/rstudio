...
export class SessionLauncher {
  ...
  private launchSession(argList: string[]): ChildProcess {
    ...
    /**
     * Check if Rosetta 2 is installed and running on Mac M1/M2.
     *
    ...
    if (process.platform === 'darwin' && process.arch === 'arm64') {
      ...
      logger().logErrorMessage('$ /usr/bin/pgrep oahd');
      let pgrepOutput = null;
      try {
        pgrepOutput = execSync('/usr/bin/pgrep oahd', { encoding: 'utf-8' });
      } catch (err: unknown) {
        logger().logDebug('Failed to verify if rosetta is installed.');
        logger().logDebug(JSON.stringify(err));
        logger().logErrorMessage(JSON.stringify(err));
        // could not verify if rosetta is installed
      }
      logger().logDebug(pgrepOutput || 'null');
      logger().logErrorMessage(pgrepOutput || 'null');
      const rosettaIsRunning = pgrepOutput !== null && pgrepOutput.trim().length > 0;
      if (!rosettaIsRunning) {
        logger().logDebug('Rosetta 2 is not running. Prompting user to install.');
        logger().logErrorMessage('Rosetta 2 is not running. Prompting user to install.');
        ...
        appState().modalTracker.trackElectronModalSync(async () =>
          dialog.showMessageBox(dialogOptions).then((selection) => {
            ...
            if (selection.response === 0) {
              ...
              const rosettaInstalledDialogOptions: MessageBoxOptions = {
                type: 'info',
                buttons: ['OK'],
                defaultId: 0,
              ...
              appState().modalTracker.trackElectronModalSync(async () =>
                dialog.showMessageBox(rosettaInstalledDialogOptions).then(() => {
                  logger().logDebug('Rosetta 2 has been installed. Please reopen RStudio.');
                  logger().logErrorMessage('Rosetta 2 has been installed. Please reopen RStudio.');
                }),
              );
              app.exit();
            } else {
              logger().logDebug('User opted out from installing Rosetta 2.');
              logger().logErrorMessage('User opted out from installing Rosetta 2.');
              const noRosettaWarning: MessageBoxOptions = {
                type: 'warning',
                buttons: ['OK'],
                defaultId: 0,
                title: 'Rosetta Is Not Installed',
              ...
              appState().modalTracker.trackElectronModalSync(async () => dialog.showMessageBox(noRosettaWarning));
            }
          }),
        );
      } else {
        // TODO: remove
        logger().logDebug('Rosetta 2 is running.');
        logger().logErrorMessage('Rosetta 2 is running.');
      }
    }
