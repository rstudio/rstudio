...
import { toReleaseSlug, isValidSlug, isReleaseBuild, resolveWhatsNewContentPath } from './whats-new-utils';
import { WhatsNewState } from './whats-new-state';
...
export class SessionLauncher {
  ...
  private whatsNewWindow: BrowserWindow | undefined;
  private pendingWhatsNew = false;
  ...
  private launchFirst(): Err {
    ...
    if (!appState().runDiagnostics) {
      ...
      this.mainWindow.window.webContents.once('did-finish-load', async () => {
        ...
        // Show What's New as a modal child of the main window
        if (this.pendingWhatsNew && this.mainWindow) {
          this.pendingWhatsNew = false;
          const info = buildInfo();
          const slug = toReleaseSlug(info.RSTUDIO_RELEASE_NAME);

          this.whatsNewWindow = showWhatsNewWindow({
            releaseSlug: slug,
            ...
            onClose: () => {
              ...
              state.markReleaseSeen(info.RSTUDIO_RELEASE_NAME, info.RSTUDIO_VERSION_PATCH);
  ...
  async showLaunchErrorPage(): Promise<void> {
    ...
    const errorUrl = getRendererUrl('src/ui/error/error.html');
    this.mainWindow
      ?.loadUrl(errorUrl)
      .then(() => {
        if (this.mainWindow) {
      ...
      .catch((reason) => {
        logger().logErrorMessage(`Failed to load ${errorUrl}: ${reason}`);
      });
  }
  ...
  /**
   * Determine whether the What's New window should be shown instead of
   * the splash screen.
   */
  private shouldShowWhatsNew(): boolean {
    ...
    // Content must exist — check slug validity and file existence
    if (!isValidSlug(slug)) {
      return false;
    }

    if (!resolveWhatsNewContentPath(slug)) {
      return false;
    }

    // RSTUDIO_SHOW_WHATS_NEW: force show (bypasses build type and seen checks)
    if (getenv('RSTUDIO_SHOW_WHATS_NEW')) {
      return true;
    }
    ...
    // Already seen this release at this patch level or higher
    const state = new WhatsNewState();
    if (state.hasSeenRelease(info.RSTUDIO_RELEASE_NAME, info.RSTUDIO_VERSION_PATCH)) {
      return false;
    }
  ...
  private onLaunchFirstSession(): void {
    ...
    const showSplash = !this.pendingWhatsNew
      && this.splashDelay > 0
      && this.showSplash
      && ElectronDesktopOptions().enableSplashScreen();
    if (showSplash) {
      // Normal splash screen path (unchanged)
