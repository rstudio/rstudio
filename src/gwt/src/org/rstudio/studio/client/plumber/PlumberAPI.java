                                   ...
                                   InterruptStatusEvent.Handler
{
   ...
   {
      if (event.getParams().getState() == PlumberAPIParams.STATE_STARTED)
      {
      ...
      else if (event.getParams().getState() == PlumberAPIParams.STATE_STOPPED)
      {
         params_ = null;
      }
   }
   ...
   {
      launchPlumberAPI(event.getPath(), event.getExtendedType());
   }
   ...
   public void onWindowClosed(WindowClosedEvent event)
   {
      ...
      if (event.getName() != PlumberAPISatellite.NAME)
         return;
   ...
   private void launchPlumberAPI(final String filePath, final String extendedType)
   {
      ...
      else if (params_ != null && isBusy_)
      {
         ...
         interrupt_.interruptR(new InterruptHandler() {
            ...
            {
               launchPlumberFile(filePath, extendedType);
            }
         });
      }
      else
      {
         ...
         dependencyManager_.withRPlumber("Running Plumber API",
               new Command() {
                  @Override
                  public void execute()
                  {
   ...
   private void notifyPlumberAPIClosed(final JavaScriptObject params)
   {
      ...
      WindowCloseMonitor.monitorSatelliteClosure(PlumberAPISatellite.NAME, new Command()
      {
         @Override
         public void execute()
         {
   ...
   private void onPlumberAPIClosed(JavaScriptObject params)
   {
      ...
      if (apiState.getState() == PlumberAPIParams.STATE_STOPPING)
      {
   ...
   private void launchPlumberFile(String file, String extendedType)
   {
