...
public class PanmirrorHTMLDialog extends ModalDialog<Boolean>
{
   ...
   {
      Element create(int conatinerWidth, int containerHeight, JsVoidFunction confirm, JsVoidFunction cancel);
   }
   ...
   public PanmirrorHTMLDialog(String title, 
                              String okText, 
                              CreateFn create,
                              JsVoidFunction focus,
                              ValidateFn validate,
                              OperationWithInput<Boolean> operation)
   {
      ...
      // create main widget
      Element mainWidgetEl = create.create(
         Window.getClientWidth(), 
         Window.getClientHeight(), 
         () -> { clickOkButton(); },
         () -> { operation.execute(false); closeDialog(); }
      );
   ...
   protected boolean validate(Boolean result)
   {
      ...
      if (error != null)
      {
         RStudioGinjector.INSTANCE.getGlobalDisplay().showErrorMessage("Error", error, new CanFocus() {
            @Override
            public void focus()
            {
               focus_.call();
