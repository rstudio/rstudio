...
public class TestthatCommandHyperlink extends Hyperlink{

    public TestthatCommandHyperlink(String url, String params, String text, String clazz) {
        super(url, params, text, clazz);
        fn_ = url.replaceFirst("ide:command:", "");
        
        String file = params_.get("file");
        if (file.matches("^\\w+$"))
        {
            code_ = fn_ + "('" + file + "')";
        }
    ...
    public Widget getPopupContent() {
        ...
        Label title = new Label(
            StringUtil.equals(fn_, "testthat::snapshot_accept") ? 
            "Accept the change" : 
            "Review the change"
        );
