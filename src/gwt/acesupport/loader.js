...
var RStudioRenderer = function(container, theme, fontMetrics) {
   ...
   if (fontMetrics) {
      ...
      self.$resizeObserver = new window.ResizeObserver(function(e) {
         // we need to do this behind requestAnimationFrame; otherwise the
         // processing time for a large number of ResizeObserver events
         // generated by e.g. opening a document with a lot of editors can
         // exceed a single animation frame, which can cause the browser to
         // emit an exception
...
(function() {
   ...
   this.onResize = function(force, gutterWidth, width, height) {
      ...
      // use synchronous resize unless this is a force render and deferred
      // resizing has been enabled (by supplying custom font metrics)
      if (!self.$deferResize || force === false) {
      ...
      if (self.container && self.container.parentElement) {
         ...
         if (pos < 2048 && self.container.parentElement.clientHeight > 0) {
            // performance: render the first (visible) chunks synchronously so
