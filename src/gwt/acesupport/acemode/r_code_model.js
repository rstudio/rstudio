...
(function () {
   ...
   function updateOldRoxygenBlock(blockRange, fnName, fnArgs)
   {
      var lines = this.$session.doc.$lines;
      
      // Find, and update, the old @param annotations.
      var startRow = blockRange.start.row;
      var endRow = blockRange.end.row;

      // Get the text for the current roxygen block.
      var roxygenBlockText = lines.slice(startRow, endRow);

      // Get the roxygen delimiter.
      var roxygenDelimiter = roxygenBlockText[0].match(/^(\s*#+'\s*)/)[1];

      // Discover the available @param annotations.
      var oldParamNames = [];
      var oldParamRowRanges = [];
      
      var reRoxygen        = /^\s*#+'/;
      var reRoxygenEmpty   = /^\s*#+'\s*$/;
      var reRoxygenParam   = /^\s*#+'\s*@param\s+([^\s]+)/;
      var reRoxygenWithTag = /^\s*#+'\s*@[^@]/;
      
      var lastParamRow = null;
      
      for (var i = 0, n = roxygenBlockText.length; i < n; i++)
      {
         var match = roxygenBlockText[i].match(reRoxygenParam);
         if (match)
         {
            lastParamRow = i;
            oldParamNames.push(match[1]);

            // Figure out where this parameter text starts and ends
            var paramRowStart = i;

            // Move off of the current tag.
            i++;

            // Find the next tag (or end of block)
            while (i < n &&
                   reRoxygen.test(roxygenBlockText[i]) &&
                   !reRoxygenWithTag.test(roxygenBlockText[i]))
            {
               i++;
            }

            // If we landed on an roxygen tag, move back one
            if (reRoxygenWithTag.test(roxygenBlockText[i]))
                i--;

            oldParamRowRanges.push({
               start: paramRowStart,
               end: i
            });
         }
      ...
      // TODO: Can we be smart enough to infer whether a particular
      // parameter was renamed (and preserve the pre-existing description)

      // Identify parameters that exist in the roxygen block, but
      // not the current set of function arguments. We want to
      // remove these.
      var paramsToRemove = [];
      var paramsToAdd    = [];
      
      for (var i = 0, n = oldParamNames.length; i < n; i++)
      {
         // If this roxygen @param name is not part of the current
         // function arguments, we want to remove it.
         if (!contains(fnArgs, oldParamNames[i]))
            paramsToRemove.push(oldParamNames[i]);
      }

      // Identify function arguments which don't exist within the
      // roxygen block. We want to add these in.
      for (var i = 0, n = fnArgs.length; i < n; i++)
      {
         if (!contains(oldParamNames, fnArgs[i]))
            paramsToAdd.push(fnArgs[i]);
      }

      // Construct a new roxygen block.
      var newRoxygenBlock = [];
      for (var i = 0, n = roxygenBlockText.length; i < n; i++)
      {
         // Skip over params we don't want.
         var match =
            roxygenBlockText[i] &&
            roxygenBlockText[i].match(reRoxygenParam);

         while (match && contains(paramsToRemove, match[1]))
         {
            i++;
            while (i < n && !reRoxygenWithTag.test(roxygenBlockText[i]))
               i++;

            match = roxygenBlockText[i].match(reRoxygenParam);
         }

         // Push the current line.
         newRoxygenBlock.push(roxygenBlockText[i]);
      }

      // Find the last '@param' annotation in our new block, and
      // append our new '@param' annotations (if necessary)
      if (paramsToAdd.length > 0)
      {
         var newParamInsertionIndex = 0;
         for (var i = 0, n = newRoxygenBlock.length; i < n; i++)
            if (reRoxygenParam.test(newRoxygenBlock[i]))
               newParamInsertionIndex = i;

         // Try to find the end of that @param annotation.
         newParamInsertionIndex++;
         while (i < n && !reRoxygenWithTag.test(newRoxygenBlock[i]))
            newParamInsertionIndex++;
         newParamInsertionIndex--;

         // Insert our new roxygen snippets at this location.
         var newRoxygenParamText = "";
         for (var i = 0, n = paramsToAdd.length; i < n; i++)
            newRoxygenParamText += "\n" + roxygenDelimiter + "@param " + paramsToAdd[i];

         newRoxygenBlock[newParamInsertionIndex] +=
            newRoxygenParamText;
         
      }

      // Ensure a space between the final @param block and the
      // next tag (if it exists)
      var n = newRoxygenBlock.length;
      for (var i = newRoxygenBlock.length; i >= 0; i--)
      {
         if (reRoxygenParam.test(newRoxygenBlock[i]))
         {
            console.log("Found param: " + newRoxygenBlock[i]);
            ...
            // Insert a newline previous if the previous block isn't empty
            console.log("Next tag: " + newRoxygenBlock[i]);
            console.log("Previous: " + newRoxygenBlock[i - 1]);
            if (!reRoxygenEmpty.test(newRoxygenBlock[i - 1]))
               newRoxygenBlock[i - 1] += "\n" + roxygenDelimiter;
      ...
      // Collapse and replace the range
      var text = newRoxygenBlock.join("\n") + "\n";
      this.$session.replace(blockRange, text);
   ...
   function generateNewRoxygenBlock(replaceRange, fnName, fnArgs)
   {
   ...
   this.insertRoxygenSkeleton = function()
   {
      ...
      if (foundBlock)
      {
         ...
         updateOldRoxygenBlock.call(this, replaceRange, fnName, fnArgs);
      }
      else
      {
         replaceRange = Range.fromPoints(fnStartPos, fnStartPos);
         generateNewRoxygenBlock.call(this, replaceRange, fnName, fnArgs);
      }
