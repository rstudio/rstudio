...
public class VirtualConsoleTests extends GWTTestCase
{
   ...
   {
      String style = "org-rstudio-studio-client-workbench-views-console-ConsoleResources-ConsoleStyles-xtermHyperlink";
      PreElement ele = Document.get().createPreElement();
      VirtualConsole vc = getVC(ele);
      String testInput = "\u001b]8;;http://www.example.com\7text\u001b]8;;\7";
      vc.submit(testInput);
      String expected = "<a class=\""+style+"\" title=\"http://www.example.com\">text</a>";
      Assert.assertEquals(expected, ele.getInnerHTML());
   }
   ...
   public void testHyperlinkNoParamsColorOutside()
   {
      ...
      vc.submit(testInput);
      String expected = "<a class=\"xtermColor1 " + style + "\" title=\"http://www.example.com\">text</a>";
      Assert.assertEquals(expected, ele.getInnerHTML());
   }
