...
aws s3 cp s3://rstudio-ide-build/version/$VERSION$BRANCH/oss-patch.csv /tmp/oss-patch.csv $EXTRA_CP_ARGS
...
if [[ $PRO = true ]]; then
    SUFFIX=0
    aws s3 cp s3://rstudio-ide-build/version/$VERSION$BRANCH/pro-suffix.csv /tmp/pro-suffix.csv --quiet
...
case "$ACTION" in
    ...
    if [[ $OPEN_SOURCE = true ]]; then
        ...
        if [[ $DEBUG = true ]]; then
            ...
            # upload to s3
            aws s3 cp /tmp/oss-updated.csv s3://rstudio-ide-build/version/$VERSION$BRANCH/oss-patch.csv --quiet
        ...
        if [[ $DEBUG = true ]]; then
            ...
            aws s3 cp /tmp/pro-updated.csv s3://rstudio-ide-build/version/$VERSION$BRANCH/pro-suffix.csv $EXTRA_CP_ARGS
