...
public class PriorityQueue<E> extends AbstractQueue<E> {
   ...
   * element), the subtrees of node i are 2*i+1 (left) and 2*i+2 (right).  Node
   * i is a leaf node if 2*i>=n.  Node i's parent, if i>0, is floor((i-1)/2).
   */
  ...
  public boolean offer(E e) {
    ...
    while (node > 0) {
      ...
      heap.set(childNode, heap.get(node)); // exchange with parent and try again
    }
    heap.set(node, e);
    return true;
  }
