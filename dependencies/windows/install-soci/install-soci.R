...
msvc <- head(Filter(file.exists, msvcpaths), n = 1)
if (length(msvc) == 0)
...
# check if we already have SOCI build
libpath <- file.path(soci_build_dir, "x64/lib/Release/libsoci_core_4_0.lib")
if (file.exists(libpath)) {
   progress(paste(libpath, "already exists; nothing to do."))
   quit()
}
