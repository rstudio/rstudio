...
- ([#16293](https://github.com/rstudio/rstudio/issues/16293)): Package origin (if known) is displayed as part of a Source column in the Packages pane in RStudio
- ([#16294](https://github.com/rstudio/rstudio/issues/16294)): An additional icon is provided in the Packages pane in RStudio, linking to a package's external documentation (e.g. `pkgdown` website) when available
- ([#16295](https://github.com/rstudio/rstudio/issues/16295)): UI is provided for changing the current repository / snapshot when using [Posit Package Manager](https://posit.co/products/enterprise/package-manager/) in RStudio
- ([#15841](https://github.com/rstudio/rstudio/issues/15841)): All keywords from the SQL 2023 standard are highlighted in SQL documents in RStudio
- ([#15919](https://github.com/rstudio/rstudio/issues/15919)): lobstr is used when computing object sizes in RStudio
- ([#16138](https://github.com/rstudio/rstudio/issues/16138)): Version 17 of the R graphics engine is supported in RStudio
- ([#16213](https://github.com/rstudio/rstudio/issues/16213)): Legibility of highlighted code is improved when the RStudio debugger is active
- ([#15945](https://github.com/rstudio/rstudio/issues/15945)): A user preference is added to disable showing the splash screen at startup in RStudio
- ([#15614](https://github.com/rstudio/rstudio/issues/15614)): The splash screen closes when users click it with the mouse in RStudio
- ([#16111](https://github.com/rstudio/rstudio/issues/16111)): The default console buffer size is increased to 10000 lines in RStudio
- ([#11661](https://github.com/rstudio/rstudio/issues/11661)): The Zoom button in the RStudio Desktop plots pane brings an existing zoomed plot window to the foreground
- ([#16009](https://github.com/rstudio/rstudio/issues/16009)): An option is added to stop ignoring update prompts when manually checking for updates (after previously ignoring an update) in RStudio
- ([#15988](https://github.com/rstudio/rstudio/issues/15988)): Posit Product Documentation theme v7.0.0 adds cookie consent, several style updates, accessibility fixes, and dark theme improvements
- ([#16214](https://github.com/rstudio/rstudio/issues/16214)): A new structure for release note entries is introduced in RStudio
- ([#16226](https://github.com/rstudio/rstudio/issues/16226)): An option (off by default) is added to zoom RStudio Desktop UI with Ctrl/Cmd+mouse wheel

#### Posit Workbench

- (rstudio-pro#8093): Workbench homepage is redesigned to provide a more efficient and intuitive user experience, with a focus on projects
- (rstudio-pro#6768): Initial support is provided for displaying Positron and VS Code projects on the homepage alongside RStudio projects in Workbench
- (rstudio-pro#8162): Support is added for displaying a custom "Metadata" column in the Packages pane in RStudio, for sessions using [Posit Package Manager](https://posit.co/products/enterprise/package-manager/)
- ([#14083](https://github.com/rstudio/rstudio/issues/14083)): Custom certificate bundles are allowed for GitHub Copilot in Workbench
- (rstudio-pro#5357): Strict enforcement of the user limit specified by the Posit Workbench product license is allowed
- (rstudio-pro#7533): Posit Product Documentation theme v7.0.1 adds cookie consent, several style updates, accessibility fixes, and dark theme improvements
- (rstudio-pro#8186): Positron Pro sessions are enabled by default in Workbench
- (rstudio-pro#7599): Existing Positron Pro configuration files are backed up and defaults are installed during upgrade in Workbench
...
- (rstudio-pro#8090): Integration with Posit Package Manager is improved in Workbench, including: repository discovery, authentication, and improved Package Manager feature visibility in the RStudio IDE
- (rstudio-pro#5068): Support is added for just-in-time provisioning of local groups for Proxied Auth, OpenID Connect, and SAML Authentication in Workbench
- (rstudio-pro#7817): R and Python scripts in Positron and VS Code have a `Run Script in a Workbench Job` action in the editor toolbar (and matching command).
- (rstudio-pro#8463): The Workbench Jobs pane in Positron and VS Code is organized by script, similar to the Explorer pane, and uses more familiar VS Code UI conventions. Clicking on a script opens it in the editor, while clicking on a job opens a virtual `Job Details` document
- (rstudio-pro#7862): Completed jobs can be individually "dismissed" from the Workbench Jobs pane in Positron and VS Code. There are also new `Dismiss Completed Jobs` and `Restore Completed Jobs` view actions to (un)hide _all_ completed jobs. Note that hidden jobs are not persistent across sessions
...
- (rstudio-pro#8289): `pwb_session_duration` is removed from Prometheus Metrics in Workbench. Session start and end times are available via the new Workbench API endpoint `get_historical_session`
- (rstudio-pro#8502): Version information for Positron and Posit Workbench Code Server is included in the output of the diagnostic report
- (rstudio-workbench-vscode-ext#278): Positron Pro sessions detect Python/R console activity even when browser is closed, preventing sessions from terminating due to inactivity
- (rstudio-pro#8504): Positron version information is displayed in Positron Pro session output in Workbench
- (vscode-server#202): GitHub Device Login url is added as a default trusted domain for VS Code and Positron Pro sessions
- (rstudio-pro#6867): Image selection in the launcher dialog is enhanced to more clearly display and classify various images to the user in Workbench
- (rstudio-pro#8776): A switch is added in both Workbench homepages to switch to either the new or legacy versions, and user choice is remembered locally (see deprecation notice)
