...
- (rstudio-pro#8920): Added option to remove recent projects from Workbench homepage; removed projects can be re-added to the homepage by opening them again
...
- (rstudio-pro#9169): Added `use-legacy-homepage` option to allow Admins to set the legacy homepage as the default during the deprecation window
...
- ([#16484](https://github.com/rstudio/rstudio/issues/16484)): Fixed an issue where the ".." symbol in the Files pane would not navigate to the parent folder in RStudio
- ([#16320](https://github.com/rstudio/rstudio/issues/16320)): Fixed an issue where the wrong message would be displayed when SSH key passwords do not match
...
- (rstudio-pro#8845): Fixed an issue where it was not possible to tab to inactive sessions on the homepage (AA)
...
- (rstudio-pro#9494): Fixed an issue where the Resume Session URL would show no available Workbenches
- (rstudio-pro#9421): Fixed an issue where Workbench Jobs for Slurm clusters could not be started from VS Code or Positron
- (rstudio-pro#9278): Fixed an issue where the VS Code and Positron Heartbeat were using the wrong environment variable to reach Workbench
- (rstudio-pro#9169): Fixed an issue where Snowflake Credentials in the New Session Dialog would show as "Invalid" instead of "Enabled" after first authentication
- (rstudio-pro#8972): Fixed an issue where SAML auth would fail when request signing was enabled
- (rstudio-pro#8968): Fixed an issue with Audited Jobs where a "Cannot allocate memory" error would sometimes occur
- (rstudio-pro#8918): Fixed an issue where duplicate project paths could prevent the legacy homepage from loading
- (rstudio-pro#8872): Fixed an issue where the session name would reset to the default when switching Workbench type in the New Session dialog
- (rstudio-pro#8046): Fixed an issue where the `rstudio-server` and `root` users were incorrectly added to Workbench's internal database
- (rstudio-pro#7638): Fixed an issue with the license tier of the Workbench API feature
- (rstudio-pro#6489): Fixed an issue where too many open files in RStudio Pro could cause the logs to fill up the entire disk
- (rstudio-pro#4901): Fixed an issue where setting `session-timeout-minutes=0` and `session-timeout-kill-hours=1` in `rsession.conf` would prevent the session from being killed correctly
- (rstudio-workbench-vscode-ext#289): Fixed an issue where the extension documentation link would link to the Posit Workbench VS Code User Guide instead of the Posit Workbench Positron User Guide

### Deprecated / Removed

#### RStudio

- ([#16485](https://github.com/rstudio/rstudio/issues/16485)): Removed inoperative min/max controls from Source Columns in RStudio
